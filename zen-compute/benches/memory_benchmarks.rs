  ...
  for (name, strategy) in strategies {
    for &size in &sizes {
      ...
      group.bench_with_input(
        ...
        |b, &size| {
          let pool =
            MemoryPool::new(strategy.clone(), 100 * 1024 * 1024).unwrap();
          b.iter(|| {
          ...
          let handles: Vec<_> = (0..num_threads)
            .map(|_| {
              ...
              thread::spawn(move || {
                ...
                for _ in 0..100 {
                  let mem: DeviceMemory<f32> =
                    pool.allocate(allocation_size).unwrap();
                  black_box(mem);
                }
              })
            })
...
fn benchmark_memory_pool_scaling(c: &mut Criterion) {
  ...
  for &pool_size in &pool_sizes {
    group.bench_with_input(
      ...
      |b, &pool_size| {
        let pool =
          MemoryPool::new(AllocationStrategy::BuddySystem, pool_size).unwrap();
