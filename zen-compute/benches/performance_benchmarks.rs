...
fn bench_monitoring_overhead(c: &mut Criterion) {
  ...
  use profiling::{CounterType, global_monitor, time_operation};
  ...
  group.bench_function("timer_creation_only", |b| {
    b.iter(|| {
      let _timer =
        time_operation(CounterType::Custom("creation_test".to_string()));
    });
  });
  ...
  for size in sizes {
    ...
    group.bench_with_input(
      ...
      |b, &size| {
        ...
        b.iter(|| {
          let result: Vec<f32> =
            data.iter().map(|&x| black_box(x * 2.0 + 1.0)).collect();
          black_box(result);
        });
      },
    );
...
mod performance_tests {
  ...
  fn test_monitoring_overhead() {
    ...
    for i in 0..iterations {
      let _timer =
        time_operation(CounterType::Custom("overhead_test".to_string()));
      black_box(i * i);
    }
    let monitored = start.elapsed();

    let overhead_ratio =
      monitored.as_nanos() as f64 / baseline.as_nanos() as f64;
    println!("Monitoring overhead ratio: {overhead_ratio:.2}x");

    // Overhead should be minimal
    assert!(
      overhead_ratio < 2.0,
      "Monitoring overhead too high: {overhead_ratio:.2}x"
    );
  }
}
