  ...
  try {
    ...
    for (let i = 0; i < Math.min(n, 1000); i++) {
      ...
      if (Math.abs(actual - expected) > tolerance) {
        if (errors < 10) {
          console.log(
            `   Error at index ${i}: expected ${expected}, got ${actual}`,
          );
        }
        errors++;
      }
    ...
    console.log(
      `   Total time: ${(copyTime + kernelTime + copyBackTime).toFixed(2)}ms`,
    );
    ...
    console.log(
      `   Effective bandwidth: ${((3 * size) / totalTime / 1e9).toFixed(2)} GB/s`,
    );
    ...
    if (transpiled.profile) {
      console.log('\n📈 Profiling Data:');
      console.log(
        `   Parse time: ${transpiled.profile.parseTime.toFixed(2)}ms`,
      );
      console.log(
        `   Transpile time: ${transpiled.profile.transpileTime.toFixed(2)}ms`,
      );
      console.log(
        `   Optimize time: ${transpiled.profile.optimizeTime.toFixed(2)}ms`,
      );
      console.log(
        `   Total transpile time: ${transpiled.profile.totalTime.toFixed(2)}ms`,
      );
    }
