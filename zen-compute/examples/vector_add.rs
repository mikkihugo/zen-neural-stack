...
use cuda_rust_wasm::{Result, kernel_function};

// Define the vector addition kernel
kernel_function!(
  VectorAddKernel,
  (&mut [f32], &[f32], &[f32], usize),
  |(c, a, b, n), ctx| {
...
fn main() -> Result<()> {
  ...
  println!(
    "Runtime initialized with device: {:?}",
    runtime.device().properties().name
  );
  ...
  println!(
    "\nAllocated {} bytes of device memory",
    n * 3 * std::mem::size_of::<f32>()
  );
  ...
  unsafe {
    ...
    launch_kernel(VectorAddKernel, config, (c_slice, a_slice, b_slice, n))?;
  }
