...
impl OptimizedWebGPUBackend {
  ...
  pub async fn with_config(config: WebGPUConfig) -> Result<Self> {
    ...
    let (device, queue) = adapter
      .request_device(
        &DeviceDescriptor {
          ...
          required_limits: Limits {
            ...
            max_compute_workgroups_per_dimension: config
      ...
      .await
      .map_err(|e| {
        CudaRustError::Backend(format!("Failed to create WebGPU device: {e}"))
      })?;
  ...
  pub fn compile_kernel(
    ...
    let _timer =
      time_operation(CounterType::Compilation).with_size(shader_source.len());
  ...
  pub async fn execute_kernel(
    ...
    let (pipeline, bind_group_layout, optimal_workgroup_size) = {
      let mut cache = self.kernel_cache.lock().unwrap();
      let cached = cache.get_mut(cache_key).ok_or_else(|| {
        CudaRustError::Backend("Kernel not found in cache".to_string())
      })?;
  ...
  fn evict_least_used_kernel(&self, cache: &mut HashMap<String, CachedKernel>) {
    if let Some((key_to_remove, _)) =
      cache.iter().min_by_key(|(_, cached)| cached.usage_count)
    {
  ...
  pub fn create_buffer(
    ...
    let _timer =
      time_operation(CounterType::MemoryAllocation).with_size(size as usize);
  ...
  pub fn performance_report(&self) -> String {
    ...
    let buffer_cache_size: usize = self
      .buffer_cache
      .lock()
      .unwrap()
      .values()
