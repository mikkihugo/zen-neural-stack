...
impl MemoryPool {
  ...
  fn preallocate_common_sizes(&self) {
    ...
    for &size in &common_sizes {
      if size >= self.config.min_pooled_size
        && size <= self.config.max_pooled_size
      {
  ...
  pub fn deallocate(&self, mut buffer: Vec<u8>) {
    ...
    // Don't pool very small or very large allocations
    if original_size < self.config.min_pooled_size
      || original_size > self.config.max_pooled_size
    {
      let mut stats = self.stats.lock().unwrap();
      stats.current_memory_usage =
        stats.current_memory_usage.saturating_sub(original_size);
      return;
    }
    ...
    if pool.len() < self.config.max_pool_size / pool_size {
      ...
      let mut stats = self.stats.lock().unwrap();
      stats.current_memory_usage =
        stats.current_memory_usage.saturating_sub(pool_size);
    }
  }
  ...
  pub fn total_pooled_memory(&self) -> usize {
    ...
    pools.iter().map(|(&size, pool)| size * pool.len()).sum()
  }
...
static GLOBAL_POOL: std::sync::OnceLock<MemoryPool> =
...
impl KernelMemoryManager {
  ...
  pub unsafe fn deallocate_kernel_memory(&self, ptr: *mut u8) -> Result<()> {
    let size = {
      let mut allocations = self.allocations.lock().unwrap();
      allocations.remove(&(ptr as *const u8)).ok_or_else(|| {
        crate::error::CudaRustError::MemoryError(
