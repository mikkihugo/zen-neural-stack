...
use super::{
  ActivationFunction, BridgeConfig, GpuDevice, NeuralBridge, NeuralOperation,
  NeuralResult, Precision,
};
...
impl BenchmarkSuite {
  ...
  fn benchmark_batch_operations(&mut self) -> NeuralResult<()> {
    ...
    for batch_size in batch_sizes {
      let result =
        self.benchmark_batch_processing(batch_size, operation_size)?;
      self.results.push(result.clone());
  ...
  fn benchmark_vector_add(&self, size: usize) -> NeuralResult<BenchmarkResult> {
    ...
    // GPU benchmark
    let gpu_time =
      self.time_operation(&self.gpu_bridge, operation.clone(), &input_data)?;

    // CPU benchmark
    let cpu_time =
      self.time_operation(&self.cpu_bridge, operation, &input_data)?;
  ...
  fn benchmark_activation_function(
    &self,
    function: ActivationFunction,
    size: usize,
  ) -> NeuralResult<BenchmarkResult> {
    let input_data: Vec<f32> =
      (0..size).map(|i| (i as f32) / 1000.0 - 5.0).collect();
  ...
  fn benchmark_batch_processing(
    ...
    let operations: Vec<_> = (0..batch_size)
      .map(|_| NeuralOperation::VectorAdd {
        size: operation_size,
        _phantom: std::marker::PhantomData,
      })
      .collect();
    ...
    let inputs: Vec<_> = (0..batch_size)
      .map(|_| {
        let mut data: Vec<f32> =
          (0..operation_size).map(|i| i as f32).collect();
    ...
    let _gpu_results =
      gpu_processor.process_batch(operations.clone(), inputs.clone())?;
  ...
  fn print_summary(&self) {
    ...
    let mut operation_groups: std::collections::HashMap<
      String,
      Vec<&BenchmarkResult>,
    ...
    for result in &self.results {
      let base_name = result
        ...
        .split('_')
        .next()
        .unwrap_or(&result.operation_name);
      operation_groups
    ...
    for (operation_type, results) in operation_groups {
      let avg_speedup: f64 =
        results.iter().map(|r| r.speedup_factor).sum::<f64>()
          / results.len() as f64;
      let max_speedup =
    ...
    let overall_avg_speedup: f64 =
      self.results.iter().map(|r| r.speedup_factor).sum::<f64>()
    ...
    println!("\n=== Performance Recommendations ===");
    let best_operations: Vec<_> = self
      .results
      .iter()
    ...
    if !best_operations.is_empty() {
      ...
      for result in best_operations {
        println!(
          "  - {} ({:.1}x speedup)",
          result.operation_name, result.speedup_factor
        );
      }
    }

    let poor_operations: Vec<_> = self
      .results
      .iter()
...
pub fn run_quick_benchmark() -> NeuralResult<()> {
  ...
  let result =
    suite.benchmark_activation_function(ActivationFunction::ReLU, 100_000)?;
