...
use super::{
  BindingType, BridgeConfig, BufferHandle, CompiledKernel, DeviceInfo,
  GpuBackendTrait, GpuDevice, NeuralIntegrationError, NeuralOperation,
...
impl GpuBackendTrait for WebGpuBackend {
  ...
  fn execute_kernel(
    ...
    let input_buffers: Vec<&wgpu::Buffer> = inputs
      ...
      .ok_or_else(|| {
        NeuralIntegrationError::OperationError(
...
fn generate_basic_wgsl(rust_code: &str) -> NeuralResult<String> {
  ...
  if rust_code.contains("matrix_multiply") || rust_code.contains("matmul") {
    Ok(
      include_str!("../webgpu/shaders/matrix_vector_multiply.wgsl").to_string(),
    )
  ...
  T: Clone + Send + Sync + 'static + num_traits::Float,
{
  match operation {
    NeuralOperation::VectorAdd { size, _phantom } => {
      if inputs.len() < size * 2 {
        return Err(NeuralIntegrationError::OperationError(
          "Insufficient input data".to_string(),
        ));
      }
        ...
        let value = match function {
          ...
          super::ActivationFunction::GELU => {
            ...
            x * T::from(0.5).unwrap_or(T::one())
              * (T::one()
                + (sqrt_2_pi
                  * (x + T::from(0.044715).unwrap_or(T::zero()) * x * x * x))
                  .tanh())
          }
        };
        result.push(value);
    ...
    NeuralOperation::MatrixMultiply {
      a_rows,
      a_cols,
      b_cols,
      _phantom,
      ...
      if inputs.len() < a_rows * a_cols + a_cols * b_cols {
        return Err(NeuralIntegrationError::OperationError(
          "Insufficient input data for matrix multiplication".to_string(),
        ));
      }
    ...
    _ => Err(NeuralIntegrationError::OperationError(format!(
      "CPU fallback not implemented for operation: {}",
      operation.name()
    ))),
  }
}
