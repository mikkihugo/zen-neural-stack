...
impl NeuralBridge {
  ...
  pub fn with_config(config: BridgeConfig) -> NeuralResult<Self> {
    ...
    let memory_manager =
      Arc::new(memory_manager::HybridMemoryManager::new(&config)?);

    // Initialize performance monitor
    let performance_monitor: Arc<dyn PerformanceMonitorTrait> =
    ...
    T: Clone + Send + Sync + 'static + bytemuck::Pod,
  {
    ...
    gpu_neural_ops::execute_operation(
      operation,
      inputs,
      backend,
      &self.memory_manager,
...
pub use bridge::{WebGpuBackend, execute_cpu_fallback, extract_wgsl_from_rust};
pub use cuda_kernels::*;
pub use gpu_neural_ops::{execute_operation, process_batch};
pub use memory_manager::HybridMemoryManager;
pub use performance_monitor::{NoOpMonitor, RealTimeMonitor};
