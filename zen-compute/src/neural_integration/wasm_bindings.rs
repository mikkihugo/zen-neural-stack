...
use super::{
  ActivationFunction, BridgeConfig, GpuDevice, MemoryStats, NeuralBridge,
  NeuralOperation, NeuralResult, PerformanceStats, Precision,
  SystemCapabilities,
};
...
impl WasmNeuralBridge {
  ...
  pub fn new() -> Result<WasmNeuralBridge, JsValue> {
    crate::neural_integration::initialize()
      .map_err(|e| JsValue::from_str(&e.to_string()))?;

    let bridge =
      NeuralBridge::new().map_err(|e| JsValue::from_str(&e.to_string()))?;
  ...
  ) -> Result<WasmNeuralBridge, JsValue> {
    crate::neural_integration::initialize()
      .map_err(|e| JsValue::from_str(&e.to_string()))?;

    let rust_config = BridgeConfig::from(config);
    let bridge = NeuralBridge::with_config(rust_config)
      .map_err(|e| JsValue::from_str(&e.to_string()))?;
...
mod tests {
  ...
  fn test_config_conversion() {
    let wasm_config =
      crate::neural_integration::wasm_types::WasmBridgeConfig::default();
