...
pub use performance_monitor::{
  CounterStats, CounterType, MonitorConfig, PerformanceMonitor,
  PerformanceReport, Timer, global_monitor, global_report, record_measurement,
  time_operation,
};
...
impl GlobalProfiler {
  ...
  pub fn export_csv(&self, path: &str) -> Result<(), CudaRustError> {
    ...
    let mut file = File::create(path).map_err(|e| {
      CudaRustError::RuntimeError(format!("Failed to create file: {e}"))
    })?;
    ...
    for profile in profiles.values() {
      ...
      .map_err(|e| {
        CudaRustError::RuntimeError(format!("Failed to write data: {e}"))
      })?;
    }
...
macro_rules! profile_scope {
  ($profiler:expr, $name:expr) => {
    let _timer =
      $crate::profiling::ScopedTimer::new($profiler, $name.to_string());
  };
}
