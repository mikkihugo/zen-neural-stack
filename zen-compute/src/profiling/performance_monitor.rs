...
impl<'a> Drop for Timer<'a> {
  fn drop(&mut self) {
    ...
    self
      .monitor
      .record_measurement(self.counter_type.clone(), measurement);
  }
}
...
impl PerformanceMonitor {
  ...
  pub fn stats(&self, counter_type: &CounterType) -> Option<CounterStats> {
    ...
    let mut durations: Vec<Duration> =
      measurements.iter().map(|m| m.duration).collect();
    durations.sort();
    ...
    let p95_time = durations
      .get(p95_index.saturating_sub(1))
      .copied()
      .unwrap_or(max_time);
    let p99_time = durations
  ...
  pub fn memory_usage(&self) -> usize {
    ...
    counters
      .values()
      .map(|measurements| {
        measurements.len() * std::mem::size_of::<Measurement>()
      })
      .sum::<usize>()
...
impl PerformanceReport {
  ...
  pub fn to_string(&self) -> String {
    ...
    for (counter_type, stats) in &self.stats {
      ...
      report
        .push_str(&format!("  Throughput: {:.2} ops/s\n", stats.throughput));
...
static GLOBAL_MONITOR: std::sync::OnceLock<PerformanceMonitor> =
...
macro_rules! time_block {
  ($counter_type:expr, $block:block) => {{
    let _timer =
      $crate::profiling::performance_monitor::time_operation($counter_type);
    $block
  }};
...
mod tests {
  ...
  fn test_timer_with_metadata() {
    ...
    {
      let _timer = monitor
        .time(CounterType::MemoryAllocation)
  ...
  fn test_time_block_macro() {
    ...
    assert!(
      report
        .stats
        .contains_key(&CounterType::Custom("test".to_string()))
    );
  }
}
