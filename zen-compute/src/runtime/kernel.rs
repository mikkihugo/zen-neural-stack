...
use super::{Block, Device, Grid, Stream};
...
impl ThreadContext {
  ...
  pub fn global_thread_id_2d(&self) -> (usize, usize) {
    let x = self.block_idx.x as usize * self.block_dim.x as usize
      + self.thread_idx.x as usize;
    let y = self.block_idx.y as usize * self.block_dim.y as usize
      + self.thread_idx.y as usize;
    (x, y)
  }
  ...
  pub fn global_thread_id_3d(&self) -> (usize, usize, usize) {
    ...
    let y = self.block_idx.y as usize * self.block_dim.y as usize
      + self.thread_idx.y as usize;
    let z = self.block_idx.z as usize * self.block_dim.z as usize
      + self.thread_idx.z as usize;
    (x, y, z)
  }
}
  ...
  ($name:ident, $args:ty, |$args_pat:pat, $ctx:ident| $body:block) => {
    ...
    impl $crate::runtime::kernel::KernelFunction<$args> for $name {
      fn execute(
        &self,
        $args_pat: $args,
        $ctx: $crate::runtime::kernel::ThreadContext,
      ) {
        $body
      }
