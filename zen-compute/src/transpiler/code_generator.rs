...
impl CodeGenerator {
  ...
  fn generate_type(&self, ty: &Type) -> Result<TokenStream> {
    match ty {
      ...
      Type::Texture(_) => {
        Err(translation_error!("Texture types not yet supported"))
      }
    }
  }
  ...
  fn generate_block(&self, block: &Block) -> Result<TokenStream> {
    let statements: Vec<TokenStream> = block
      .statements
      .iter()
  ...
  fn generate_statement(&self, stmt: &Statement) -> Result<TokenStream> {
    match stmt {
      ...
      Statement::SyncThreads => {
        Ok(quote! { cuda_rust_wasm::runtime::sync_threads(); })
      }
    }
  }

  /// Generate storage class attributes
  fn generate_storage_class(
    &self,
      ...
      Expression::WarpPrimitive { op, args } => {
        ...
        match op {
          WarpOp::Shuffle => {
            if args.len() != 2 {
              return Err(translation_error!(
                "Warp shuffle requires 2 arguments"
              ));
            }
          ...
          WarpOp::ShuffleXor => {
            if args.len() != 2 {
              return Err(translation_error!(
                "Warp shuffle_xor requires 2 arguments"
              ));
            }
            let value = self.generate_expression(&args[0])?;
            let mask = self.generate_expression(&args[1])?;
            Ok(
              quote! { cuda_rust_wasm::runtime::warp_shuffle_xor(#value, #mask) },
            )
          }
          WarpOp::ShuffleUp => {
            if args.len() != 2 {
              return Err(translation_error!(
                "Warp shuffle_up requires 2 arguments"
              ));
            }
            let value = self.generate_expression(&args[0])?;
            let delta = self.generate_expression(&args[1])?;
            Ok(
              quote! { cuda_rust_wasm::runtime::warp_shuffle_up(#value, #delta) },
            )
          }
          WarpOp::ShuffleDown => {
            if args.len() != 2 {
              return Err(translation_error!(
                "Warp shuffle_down requires 2 arguments"
              ));
            }
            let value = self.generate_expression(&args[0])?;
            let delta = self.generate_expression(&args[1])?;
            Ok(
              quote! { cuda_rust_wasm::runtime::warp_shuffle_down(#value, #delta) },
            )
          }
          WarpOp::Vote => {
            if args.len() != 1 {
          ...
          WarpOp::Ballot => {
            if args.len() != 1 {
              return Err(translation_error!(
                "Warp ballot requires 1 argument"
              ));
            }
            let predicate = self.generate_expression(&args[0])?;
          ...
          WarpOp::ActiveMask => {
            if !args.is_empty() {
              return Err(translation_error!(
                "Warp activemask takes no arguments"
              ));
            }
  ...
  fn generate_unary_op(&self, op: &UnaryOp) -> Result<TokenStream> {
    Ok(match op {
      ...
      UnaryOp::PostInc => {
        return Err(translation_error!("Post-increment not supported"));
      }
      UnaryOp::PostDec => {
        return Err(translation_error!("Post-decrement not supported"));
      }
      UnaryOp::Deref => quote! { * },
