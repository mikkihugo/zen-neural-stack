...
impl KernelTranslator {
  ...
  pub fn translate_matrix_mul(
    ...
    if kernel.params.len() < 5 {
      return Err(translation_error!(
        "Matrix multiplication requires at least 5 parameters"
      ));
    }
  ...
  fn is_vector_pattern(&self, kernel: &KernelDef) -> bool {
    ...
    kernel.params.len() >= 3 && self.has_linear_indexing(&kernel.body)
  }
  ...
  fn is_matrix_pattern(&self, kernel: &KernelDef) -> bool {
    ...
    kernel.params.len() >= 5 && self.has_2d_indexing(&kernel.body)
  }
  ...
  fn is_reduction_pattern(&self, kernel: &KernelDef) -> bool {
    ...
    self.has_shared_memory(&kernel.body) && self.has_sync_threads(&kernel.body)
  }
  ...
  fn has_2d_indexing(&self, block: &Block) -> bool {
    ...
    let has_x = block
      .statements
      .iter()
      .any(|stmt| self.uses_dimension(stmt, &Dimension::X));
    let has_y = block
      .statements
      .iter()
      .any(|stmt| self.uses_dimension(stmt, &Dimension::Y));
    has_x && has_y
  }
  ...
  fn has_sync_threads(&self, block: &Block) -> bool {
    block
      .statements
      .iter()
      .any(|stmt| matches!(stmt, Statement::SyncThreads))
  }
  ...
  fn has_neighbor_access(&self, block: &Block) -> bool {
    ...
    block
      .statements
      .iter()
      .any(|stmt| self.has_offset_access(stmt))
  }
  ...
  fn uses_dimension(&self, stmt: &Statement, dim: &Dimension) -> bool {
    match stmt {
      Statement::VarDecl {
        init: Some(expr), ..
      } => self.expr_uses_dimension(expr, dim),
  ...
  fn expr_uses_dimension(&self, expr: &Expression, dim: &Dimension) -> bool {
    match expr {
      Expression::ThreadIdx(d)
      | Expression::BlockIdx(d)
      | Expression::BlockDim(d)
      | Expression::GridDim(d) => d == dim,
      Expression::Binary { left, right, .. } => {
        self.expr_uses_dimension(left, dim)
          || self.expr_uses_dimension(right, dim)
      }
      _ => false,
    }
  }
  ...
  fn has_offset_access(&self, stmt: &Statement) -> bool {
    match stmt {
      ...
      Statement::VarDecl {
        init: Some(expr), ..
      } => self.expr_has_offset_access(expr),
      _ => false,
    }
  }
