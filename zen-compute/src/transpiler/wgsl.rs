...
impl WgslGenerator {
      ...
      if let Item::Kernel(kernel) = item {
        ...
        for param in &kernel.params {
          if matches!(param.ty, Type::Pointer(_)) {
            self.writeln(&format!("@group(0) @binding({binding_index})"))?;
            ...
            let buffer_type = match &param.ty {
              Type::Pointer(inner) => {
                ...
                if param
                  .qualifiers
                  .iter()
                  .any(|q| matches!(q, ParamQualifier::Const))
                {
                  format!(
                    "var<storage, read> {}: array<{}>;",
                    param.name, wgsl_type
                  )
                } else {
                  format!(
          ...
          match init.as_ref() {
            ...
            _ => {
              return Err(translation_error!(
                "Invalid init statement in for loop"
              ));
            }
          }
        }
  ...
  fn type_to_wgsl(&self, ty: &Type) -> Result<String> {
    Ok(match ty {
      Type::Void => {
        return Err(translation_error!("void type not supported in WGSL"));
      }
      Type::Bool => "bool".to_string(),
      Type::Int(int_ty) => match int_ty {
        IntType::I8 | IntType::I16 | IntType::I32 => "i32".to_string(),
        IntType::I64 => {
          return Err(translation_error!("i64 not supported in WGSL"));
        }
        IntType::U8 | IntType::U16 | IntType::U32 => "u32".to_string(),
        IntType::U64 => {
          return Err(translation_error!("u64 not supported in WGSL"));
        }
      },
      Type::Float(float_ty) => match float_ty {
        ...
        FloatType::F64 => {
          return Err(translation_error!("f64 not supported in WGSL"));
        }
      },
      Type::Pointer(inner) => {
      ...
      Type::Texture(_) => {
        return Err(translation_error!("Texture types not yet supported"));
      }
    })
  }
  ...
  fn unary_op_to_wgsl(&self, op: &UnaryOp) -> Result<&'static str> {
    Ok(match op {
      ...
      UnaryOp::BitNot => "~",
      UnaryOp::PreInc => {
        return Err(translation_error!("Pre-increment not supported in WGSL"));
      }
      UnaryOp::PreDec => {
        return Err(translation_error!("Pre-decrement not supported in WGSL"));
      }
      UnaryOp::PostInc => {
        return Err(translation_error!("Post-increment not supported in WGSL"));
      }
      UnaryOp::PostDec => {
        return Err(translation_error!("Post-decrement not supported in WGSL"));
      }
      UnaryOp::Deref => "*",
      UnaryOp::AddrOf => "&",
    })
