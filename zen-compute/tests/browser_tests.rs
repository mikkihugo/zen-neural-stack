...
mod browser_tests {
  ...
  fn test_webassembly_simd_support() {
    ...
    let has_simd = js_sys::WebAssembly::validate(&[
      ...
      0x00, 0x00, 0x00, 0x00, 0x1a, 0x0b, // drop, end
    ]);
  ...
  fn test_performance_in_browser() {
    ...
    launcher
      .launch("compute_intensive", config, &[d_data.as_arg(), n.as_arg()])
      .unwrap();
    ...
    console::log_1(
      &format!("Compute intensive kernel took: {} ms", duration_ms).into(),
    );
  ...
  fn test_memory_limits_in_browser() {
    ...
    for size in sizes {
      ...
      match mem {
        Ok(_) => console::log_1(
          &format!("Successfully allocated {} bytes", size).into(),
        ),
        Err(e) => console::log_1(
          &format!("Failed to allocate {} bytes: {}", size, e).into(),
        ),
      }
    }
  }
  ...
  fn test_error_handling_in_browser() {
    ...
    console::log_1(
      &format!("Expected error caught: {}", result.unwrap_err()).into(),
    );
  }
  ...
  fn test_canvas_integration() {
      ...
      js_sys::Reflect::get(&window.navigator(), &JsValue::from_str("gpu"))
    {
      if !gpu.is_undefined() {
        ...
        match runtime {
          Ok(_) => {
            console::log_1(&"WebGPU runtime with canvas initialized".into())
          }
          Err(e) => {
            console::log_1(&format!("WebGPU canvas init failed: {}", e).into())
          }
        }
      }
    }
...
mod browser_simulation_tests {
  ...
  fn test_browser_memory_constraints() {
    ...
    let runtime = WasmRuntime::new(options).unwrap();
    let pool =
      MemoryPool::new(AllocationStrategy::BestFit, small_limit).unwrap();
    ...
    let large_alloc: Result<DeviceMemory<f32>, _> =
