...
mod cross_platform_tests {
  ...
  fn test_macos_specific_features() {
    ...
    match runtime {
      Ok(rt) => {
        assert!(
          rt.get_backend_info().name.contains("Metal")
  ...
  fn test_windows_specific_features() {
    ...
    match runtime {
      Ok(rt) => {
        ...
        assert!(
          backend.name.contains("DirectX") || backend.name.contains("WASM")
        );
      }
  ...
  fn test_endianness_handling() {
    ...
    let pool =
      MemoryPool::new(AllocationStrategy::BestFit, 1024 * 1024).unwrap();
  ...
  fn test_thread_safety_across_platforms() {
    ...
    let runtime =
      Arc::new(WasmRuntime::new(RuntimeOptions::default()).unwrap());
    let num_threads =
      std::thread::available_parallelism().unwrap().get().min(8);
    ...
    let handles: Vec<_> = (0..num_threads)
      .map(|_| {
        ...
        thread::spawn(move || {
          ...
          for _ in 0..10 {
            let pool =
              MemoryPool::new(AllocationStrategy::BestFit, 1024 * 1024)
                .unwrap();
  ...
  fn test_compilation_feature_detection() {
    ...
    assert!(
      features.basic_compute,
      "Basic compute should always be available"
    );
  }
  ...
  fn test_system_resource_integration() {
    ...
    if safe_allocation > 100 * 1024 * 1024 {
      // If more than 100MB available
