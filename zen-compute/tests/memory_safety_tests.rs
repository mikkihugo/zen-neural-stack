...
use cuda_rust_wasm::{
  error::CudaError,
  memory::{
    AllocationStrategy, DeviceMemory, MemoryPool, PinnedMemory, UnifiedMemory,
  },
...
mod memory_safety_tests {
  ...
  fn test_memory_leak_detection() {
    ...
    for _ in 0..100 {
      let pool =
        MemoryPool::new(AllocationStrategy::BestFit, 10 * 1024 * 1024).unwrap();
  ...
  fn test_concurrent_memory_safety() {
    let pool = Arc::new(
      MemoryPool::new(AllocationStrategy::BuddySystem, 100 * 1024 * 1024)
        .unwrap(),
    );
    let num_threads = 8;
  ...
  fn test_memory_alignment_safety() {
    ...
    for alignment in alignments {
      ...
      if let Ok(runtime) = runtime {
        let pool =
          MemoryPool::new(AllocationStrategy::BestFit, 10 * 1024 * 1024)
            .unwrap();
  ...
  fn test_memory_pressure_handling() {
    ...
    let small_pool =
      MemoryPool::new(AllocationStrategy::BestFit, 1024 * 1024).unwrap(); // 1MB
    let mut allocations = Vec::new();
    ...
    for i in 0..1000 {
      ...
      match small_pool.allocate::<f32>(size) {
        ...
        Err(CudaError::OutOfMemory) => {
          println!(
            "Hit memory limit after allocating {} bytes",
            total_allocated
          );
          break;
        }
  ...
  fn test_memory_pool_isolation() {
    ...
    let pool1 =
      MemoryPool::new(AllocationStrategy::BestFit, 5 * 1024 * 1024).unwrap();
    let pool2 =
      MemoryPool::new(AllocationStrategy::FirstFit, 5 * 1024 * 1024).unwrap();
  ...
  fn test_resource_cleanup_on_panic() {
    ...
    assert_eq!(
      initial_stats.allocated_bytes, final_stats.allocated_bytes,
  ...
  fn test_stack_overflow_protection() {
    ...
    fn recursive_allocation(
      pool: &MemoryPool,
      depth: usize,
    ) -> Result<Vec<DeviceMemory<f32>>, CudaError> {
      if depth == 0 {
        return Ok(Vec::new());
      }
    ...
    match result {
      ...
      Err(CudaError::OutOfMemory) => {
        println!("Deep recursion limited by memory")
      }
