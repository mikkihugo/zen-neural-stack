...
mod memory_tests {
  ...
  use cuda_rust_wasm::memory::{
    AllocationStrategy, DeviceMemory, MemoryInfo, MemoryPool, PinnedMemory,
    UnifiedMemory,
  };
  use std::sync::Arc;
  use std::thread;
  ...
  fn test_memory_fragmentation() {
    let pool =
      MemoryPool::new(AllocationStrategy::FirstFit, 1024 * 1024).unwrap();
  ...
  fn test_concurrent_allocation() {
    let pool = Arc::new(
      MemoryPool::new(AllocationStrategy::BuddySystem, 10 * 1024 * 1024)
        .unwrap(),
    );
  ...
  fn test_memory_alignment() {
    ...
    for alignment in alignments {
      let mem: DeviceMemory<u8> =
        pool.allocate_aligned(1000, alignment).unwrap();
