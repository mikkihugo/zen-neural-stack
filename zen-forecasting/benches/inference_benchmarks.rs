...
fn bench_streaming_prediction(c: &mut Criterion) {
  ...
  let stream_lengths = vec![("short", 100), ("medium", 1000), ("long", 10000)];
  ...
  for (stream_name, stream_length) in stream_lengths {
    ...
    let full_data =
      generate_inference_data(1, stream_length + window_size, 1)[0].clone();
...
fn bench_multistep_forecasting(c: &mut Criterion) {
  ...
  let forecast_horizons =
    vec![("short", 12), ("medium", 24), ("long", 48), ("xlong", 96)];
      ...
      group.bench_with_input(
        ...
        |b, data| {
          b.iter(|| {
            let predictions: Vec<_> = data
              .iter()
              .map(|series| model.predict_multistep(black_box(series), horizon))
              .collect();
            black_box(predictions);
          });
        },
        ...
        b.iter(|| {
          if num_threads == 1 {
            ...
            let predictions: Vec<_> =
              data.iter().map(|series| model.predict(series)).collect();
            black_box(predictions);
          } else {
            // Parallel processing
...
fn bench_first_prediction_latency(c: &mut Criterion) {
  ...
  let model_configs = vec![
    (
      "MLP_small",
      MLPConfig::new(24, 12).with_hidden_layers(vec![32]),
    ),
    (
      "MLP_large",
...
fn bench_ensemble_inference(c: &mut Criterion) {
  ...
  let ensemble_sizes = vec![("small", 3), ("medium", 5), ("large", 10)];

  let data = generate_inference_data(50, 48, 1);

  for (size_name, num_models) in ensemble_sizes {
    let mut models: Vec<Box<dyn BaseModel<f64>>> =
      Vec::with_capacity(num_models);
    ...
    group.bench_with_input(
      ...
      |b, models| {
        b.iter(|| {
          let ensemble_predictions: Vec<Vec<_>> = data
            .iter()
            .map(|series| {
              models
                .iter()
          ...
          // Average predictions
          let final_predictions: Vec<_> = ensemble_predictions
            .iter()
            .map(|preds| {
              let sum: f64 = preds.iter().map(|p| p.point_forecast).sum();
...
#[cfg(feature = "parallel")]
criterion_group!(parallel_benches, bench_parallel_inference,);
