...
use neuro_divergent::{
  Frequency, NeuralForecast, PredictionIntervals, ScalerType,
  config::{
    Device, EarlyStoppingConfig, IntervalMethod, LossFunction, OptimizerType,
    SchedulerConfig, SchedulerType,
  },
  data::{
    DataPreprocessor, FeatureEngineer, RobustScaler, StandardScaler,
    TimeSeriesDataFrame, TimeSeriesSchema,
  },
  models::{DeepAR, LSTM, NBEATS, TFT, Transformer},
  training::{AccuracyMetrics, HyperparameterTuner, SearchSpace},
};
use polars::prelude::*;
...
fn main() -> Result<()> {
  ...
  println!(
    "   ✅ Generated {} series with exogenous variables",
    data.n_series()?
  );
  ...
  println!(
    ...
    engineered_data.schema.all_columns().len()
      - data.schema.all_columns().len()
  );
  ...
  // Split data
  let (train_data, test_data) = preprocessed_data
    .train_test_split_by_date(Utc::now() - Duration::days(30))?;

  // 4. Hyperparameter Tuning
  println!("\n4️⃣ Performing hyperparameter tuning...");
  let best_models = tune_hyperparameters(&train_data)?;
  println!(
    "   ✅ Found optimal hyperparameters for {} models",
    best_models.len()
  );
  ...
  let mut multi_series_nf =
    train_multi_series_models(advanced_models, &train_data)?;

  // 7. Probabilistic Forecasting
  println!("\n7️⃣ Generating probabilistic forecasts...");
  let probabilistic_forecasts =
    generate_probabilistic_forecasts(&multi_series_nf, &test_data)?;
...
fn generate_advanced_dataset() -> Result<TimeSeriesDataFrame<f32>> {
  ...
  for store_id in 0..n_stores {
    ...
    for product_id in 0..n_products {
      ...
      for day in 0..n_days {
        ...
        let weekly_pattern = 1.0
          + 0.3 * (2.0 * std::f64::consts::PI * day_of_week as f64 / 7.0).sin();
        let monthly_pattern = 1.0
          + 0.2
            * (2.0 * std::f64::consts::PI * day_of_month as f64 / 30.0).sin();
        let yearly_pattern =
          1.0 + 0.4 * (2.0 * std::f64::consts::PI * day as f64 / 365.0).sin();

        // External factors
        let temperature = 20.0
          + 15.0 * (2.0 * std::f64::consts::PI * day as f64 / 365.0).cos()
        ...
        let discount_pct = if promotion {
          rng.gen_range(0.1..0.3)
        } else {
          0.0
        };
        let advertising_spend = if promotion {
        ...
        let weather_factor =
          1.0 - precipitation / 100.0 + (temperature - 20.0) / 50.0;
  ...
  let schema = TimeSeriesSchema {
    ...
    .map(String::from)
    .collect(),
    static_cols: vec!["store_type", "store_location", "product_category"]
      .into_iter()
      .map(String::from)
      .collect(),
  };
...
) -> Result<TimeSeriesDataFrame<f32>> {
  ...
  let pipeline = PreprocessingPipeline::builder()
    ...
    .add_step(
      GroupScaler::new()
        ...
        .add_group(
          "weather_features",
          vec!["temperature", "precipitation"],
          RobustScaler::new(),
        )
        .add_group(
          "marketing_features",
          vec!["discount_pct", "advertising_spend"],
...
fn tune_hyperparameters(
  ...
  // Tune LSTM
  println!("   Tuning LSTM...");
  let best_lstm = tuner.optimize(|| LSTM::builder(), lstm_space, train_data)?;
  optimized_models.push(best_lstm);

  // Tune NBEATS
  println!("   Tuning NBEATS...");
  let best_nbeats =
    tuner.optimize(|| NBEATS::builder(), nbeats_space, train_data)?;
  ...
  let models: Vec<Box<dyn BaseModel<f32>>> = vec![
    ...
    Box::new(
      LSTM::builder()
        ...
        .static_features(
          vec!["store_type", "store_location", "product_category"]
            .into_iter()
            .map(String::from)
            .collect(),
        )
    ...
    Box::new(
      DeepAR::builder()
        ...
        .learning_rate(0.001)
        .hist_exog_features(
          vec!["temperature", "promotion", "advertising_spend"]
            .into_iter()
            .map(String::from)
            .collect(),
        )
...
fn train_multi_series_models(
  ...
  let callbacks = vec![
    ...
    Box::new(
      ModelCheckpoint::new("./checkpoints")
        .monitor("val_loss")
        .save_best_only(true),
    ),
...
fn analyze_prediction_intervals(
...
fn demonstrate_transfer_learning(
  train_data: &TimeSeriesDataFrame<f32>,
) -> Result<()> {
...
fn perform_advanced_evaluation(
  ...
  println!(
    "      - Quantile Loss: {:.3}",
    advanced_metrics.quantile_loss()
  );
...
fn generate_realtime_observation(
  index: usize,
) -> Result<TimeSeriesObservation> {
