...
use neuro_divergent::{
  ...
  config::{
    CrossValidationConfig, Device, EarlyStoppingConfig, EarlyStoppingMode,
    IntervalMethod, LossFunction, OptimizerType,
  },
  data::{
    DataPreprocessor, DataValidator, StandardScaler, TimeSeriesDataFrame,
    TimeSeriesSchema, ValidationReport,
  },
  models::{DeepAR, LSTM, MLP, NBEATS},
...
fn main() -> Result<()> {
  ...
  let data =
    load_time_series_data("data/sales_data.csv").unwrap_or_else(|_| {
      println!(
        "   ⚠️  Could not load CSV file, generating synthetic data instead"
      );
  ...
  println!(
    "   ✅ Loaded {} series with {} total observations",
    data.n_series()?,
    data.shape().0
  );
  ...
  let cv_results =
    run_cross_validation(&mut neural_forecast, train_data.clone())?;
  ...
  println!(
    "   ✅ Generated forecasts for {} periods",
    forecasts.horizon()
  );
...
fn generate_synthetic_sales_data() -> Result<TimeSeriesDataFrame<f32>> {
  ...
  for store_id in 0..n_stores {
    ...
    for day in 0..n_days {
      ...
      let weekly_seasonality =
        200.0 * (2.0 * std::f64::consts::PI * day as f64 / 7.0).sin();
      let yearly_seasonality =
        500.0 * (2.0 * std::f64::consts::PI * day as f64 / 365.0).sin();
