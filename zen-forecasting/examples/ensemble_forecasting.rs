...
use neuro_divergent::{
  ...
  config::{IntervalMethod, LossFunction, OptimizerType, PredictionIntervals},
  data::{TimeSeriesDataFrame, TimeSeriesSchema},
  models::{DeepAR, LSTM, MLP, NBEATS, NHITS, Transformer},
...
fn main() -> Result<()> {
  ...
  println!(
    "   ✅ Split data: train={}, val={}, test={} rows",
    train_data.shape().0,
    val_data.shape().0,
    test_data.shape().0
  );
  ...
  let ensemble_metrics =
    AccuracyMetrics::calculate(&test_data, &ensemble_forecast)?;
...
fn create_diverse_ensemble() -> Result<Vec<(String, Box<dyn BaseModel<f32>>)>> {
  let models = vec![
    ...
    (
      "NBEATS_interpretable".to_string(),
      Box::new(
        NBEATS::builder()
          .horizon(14)
          .input_size(28)
...
fn create_hierarchical_ensemble(
  ...
  let models = vec![
    Box::new(LSTM::builder().horizon(14).input_size(28).build()?)
      as Box<dyn BaseModel<f32>>,
    Box::new(NBEATS::interpretable().horizon(14).input_size(28).build()?),
    Box::new(MLP::builder().horizon(14).input_size(28).build()?),
  ];
...
fn generate_probabilistic_ensemble(
  ...
  let intervals =
    PredictionIntervals::new(quantiles.clone(), IntervalMethod::Quantile)?;
...
fn print_validation_performance(
  performance: &HashMap<String, ModelPerformance>,
) {
...
fn compare_ensemble_vs_individual(
  ...
  let improvement =
    (best_individual.mae - ensemble.mae()) / best_individual.mae * 100.0;
...
impl PredictionConfig {
  ...
  fn with_intervals(self) -> Self {
    self
  }
  fn with_num_samples(self, _: usize) -> Self {
    self
  }
}
