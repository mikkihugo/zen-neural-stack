  ...
  pub fn validate_dataframe(&self, df: &DataFrame) -> NeuroDivergentResult<()> {
    ...
    if !columns.contains(&self.unique_id_col) {
      return Err(data_error!(
        format!(
          "Required unique_id column '{}' not found",
          self.unique_id_col
        ),
        field = "unique_id_col"
      ));
...
impl<T: Float> TimeSeriesDataFrame<T> {
  ...
  pub fn from_parquet<P: AsRef<Path>>(
    ...
    let df = LazyFrame::scan_parquet(path, ScanArgsParquet::default())
      ...
      .collect()
      .map_err(|e| {
        ErrorBuilder::data(format!("Failed to collect DataFrame: {}", e))
          .build()
      })?;
  ...
  pub fn to_dataset(&self) -> NeuroDivergentResult<TimeSeriesDataset<T>> {
    ...
    Ok(TimeSeriesDataset {
      unique_ids,
      series_data,
      static_features: if static_features.is_empty() {
        None
      } else {
        Some(static_features)
      },
      schema: self.schema.clone(),
      metadata,
    })
  ...
  fn extract_series_data(
    ...
    df: &DataFrame,
    _id: &str,
  ) -> NeuroDivergentResult<SeriesData<T>> {
    // Extract timestamps
  ...
  fn compute_metadata(
    ...
    Ok(DatasetMetadata {
      ...
      avg_series_length,
      min_series_length: if min_length == usize::MAX {
        0
      } else {
        min_length
      },
...
impl<T: Float> TimeSeriesDatasetBuilder<T> {
  ...
  pub fn build(self) -> NeuroDivergentResult<TimeSeriesSchema> {
    let unique_id_col = self
      .unique_id_col
      .ok_or_else(|| data_error!("Unique ID column must be specified"))?;

    let ds_col = self
      .ds_col
      .ok_or_else(|| data_error!("Timestamp column must be specified"))?;

    let y_col = self
      .y_col
      .ok_or_else(|| data_error!("Target column must be specified"))?;
