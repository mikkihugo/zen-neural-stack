...
use crate::{
  data::{SeriesData, TimeSeriesDataFrame, TimeSeriesDataset},
  error::{
    ErrorBuilder, NetworkIntegrationError, NeuroDivergentError,
...
pub trait InputPreprocessor<T: Float + Send + Sync + 'static>:
  Send + Sync
{
...
pub trait OutputPostprocessor<T: Float + Send + Sync + 'static>:
  Send + Sync
{
  ...
  fn process(
    &self,
    output: &[T],
    context: &PostprocessorContext<T>,
...
impl<T: Float + Send + Sync + 'static + 'static> NetworkAdapter<T> {
  ...
  pub fn forward(
    &mut self,
    input: &TimeSeriesInput<T>,
  ) -> NeuroDivergentResult<NetworkOutput<T>> {
    // Preprocess input
    let processed_input =
    ...
    if processed_input.len() != self.config.input_size {
      return Err(
        NetworkIntegrationError::ArchitectureMismatch {
  ...
  pub fn new(algorithm: Box<dyn TrainingAlgorithm<T>>) -> Self {
    let config = TrainingBridgeConfig {
      ...
      learning_rate: T::from(0.001)
        .unwrap_or_else(|| T::one() / T::from(1000).unwrap()),
      target_error: T::from(0.001)
        .unwrap_or_else(|| T::one() / T::from(1000).unwrap()),
  ...
  fn prepare_training_data(
    ...
    for series in dataset.series_data.values() {
      ...
      for i in 0..(series
        .target_values
        .len()
        .saturating_sub(window_size + horizon))
      {
        let input_window = &series.target_values[i..i + window_size];
        let output_window =
          &series.target_values[i + window_size..i + window_size + horizon];
...
impl ActivationMapper {
  ...
  pub fn new() -> Self {
    ...
    function_map
      .insert("linear_piece".to_string(), ActivationFunction::LinearPiece);
    function_map.insert(
      "linear_piece_symmetric".to_string(),
...
impl<T: Float + Send + Sync + 'static> InputPreprocessor<T>
  for DefaultInputPreprocessor<T>
{
  fn process(
    &self,
    input: &TimeSeriesInput<T>,
    ...
    Ok(
      input.target_history[input.target_history.len() - window_size..].to_vec(),
    )
  }
...
impl<T: Float + Send + Sync + 'static> OutputPostprocessor<T>
  for DefaultOutputPostprocessor<T>
{
  fn process(
    &self,
    output: &[T],
...
mod tests {
  ...
  fn test_activation_mapper() {
    ...
    assert_eq!(
      mapper.map_function("sigmoid"),
      Some(ActivationFunction::Sigmoid)
    );
    assert_eq!(
      mapper.map_function("linear"),
  ...
  fn test_configuration_serialization() {
    ...
    let deserialized: NetworkAdapterConfig<f64> =
