...
use crate::{
  plugin::PluginManager, ModelCapabilities, ModelCategory, ModelInfo,
  ModelPerformance, RegistryResult,
};
...
impl ModelDiscovery {
  ...
  pub fn discover_all(&self) -> RegistryResult<DiscoveryResult> {
    ...
    if config.detect_capabilities {
      for model in &mut models {
        if let Err(e) = self.detect_model_capabilities(model) {
          errors.push(DiscoveryError {
            message: format!(
              "Failed to detect capabilities for {}: {}",
              model.name, e
            ),
      ...
      stats.with_capabilities = models
        .iter()
    ...
    if config.profile_performance {
      for model in &mut models {
        if let Err(e) = self.profile_model_performance(model) {
          errors.push(DiscoveryError {
            message: format!(
              "Failed to profile performance for {}: {}",
              model.name, e
            ),
            location: model.name.clone(),
      ...
      stats.with_performance =
        models.iter().filter(|m| m.performance.is_some()).count();
    }
    ...
    log::info!(
      "Model discovery completed: {} models found in {:.2}ms",
      models.len(),
      duration
    );
  ...
  /// Discover built-in models
  fn discover_builtin_models(
    &self,
    _config: &DiscoveryConfig,
  ) -> RegistryResult<Vec<ModelInfo>> {
    let mut models = Vec::new();
  ...
  fn create_specialized_models(&self) -> Vec<ModelInfo> {
    vec![
      ...
      self.create_model_info(
        "TimesNet",
        ModelCategory::Specialized,
        "TimesNet: Temporal 2D-Variation",
      ),
      self.create_model_info(
        "StemGNN",
        ModelCategory::Specialized,
  ...
  fn get_default_metadata(
    ...
    let tags = match name {
      ...
      "TFT" | "Informer" | "AutoFormer" | "FedFormer" | "PatchTST"
      | "iTransformer" => vec!["transformer", "attention", "self-attention"],
      "TCN" | "BiTCN" => vec!["convolutional", "temporal", "causal"],
      "NBEATS" | "NBEATSx" => {
        vec!["interpretable", "basis-expansion", "hierarchical"]
      }
  ...
  /// Discover plugin models
  fn discover_plugin_models(
    &self,
    config: &DiscoveryConfig,
  ) -> RegistryResult<(Vec<ModelInfo>, PluginDiscoveryStats)> {
    let mut models = Vec::new();
    ...
    log::debug!(
      "Discovered {} plugin models from {} plugins",
      models.len(),
    ...
    if !filters.include_names.is_empty() {
      models.retain(|m| {
        filters
          .include_names
          .iter()
          .any(|pattern| self.matches_pattern(&m.name, pattern))
      });
    }
  ...
  fn extract_capability_names(
    &self,
    capabilities: &ModelCapabilities,
  ) -> HashSet<String> {
    let mut caps = HashSet::new();

    if capabilities.online_learning {
      caps.insert("online_learning".to_string());
    }
    if capabilities.batch_processing {
      caps.insert("batch_processing".to_string());
    }
    if capabilities.streaming {
      caps.insert("streaming".to_string());
    }
    if capabilities.multi_threading {
      caps.insert("multi_threading".to_string());
    }
    if capabilities.gpu_acceleration {
      caps.insert("gpu_acceleration".to_string());
    }
    if capabilities.quantization {
      caps.insert("quantization".to_string());
    }
    if capabilities.pruning {
      caps.insert("pruning".to_string());
    }
    if capabilities.fine_tuning {
      caps.insert("fine_tuning".to_string());
    }
  ...
  fn cache_discovery_results(&self, models: &[ModelInfo]) {
    ...
    for model in models {
      cache
        .models
        .insert(model.name.clone(), (model.clone(), now));
    }
...
pub fn discover_builtin_models_with_config(
  config: &DiscoveryConfig,
) -> RegistryResult<Vec<ModelInfo>> {
...
pub fn discover_all_models_with_config(
  config: DiscoveryConfig,
) -> RegistryResult<DiscoveryResult> {
...
mod tests {
  ...
  fn test_default_capabilities_by_category() {
    ...
    let recurrent_caps =
      discovery.get_default_capabilities(ModelCategory::Recurrent);
    assert!(recurrent_caps.streaming);
    assert!(!recurrent_caps.multi_threading); // Sequential nature

    let transformer_caps =
      discovery.get_default_capabilities(ModelCategory::Transformer);
