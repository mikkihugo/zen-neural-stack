...
use crate::{
  global_registry, BaseModel, Float, ModelCategory, ModelConfig, ModelInfo,
  ModelPerformance, RegistryError, RegistryResult,
};
use instant::Instant;
...
pub type ModelCreatorFn<T> = Box<
  dyn Fn(&ModelConfig) -> RegistryResult<Box<dyn BaseModel<T>>> + Send + Sync,
>;
...
pub struct ModelFactory {
  ...
  creators: RwLock<
    HashMap<
      String,
      Box<
        dyn Fn(&ModelConfig) -> RegistryResult<Box<dyn BaseModel<f32>>>
          + Send
          + Sync,
      >,
    >,
  ...
  creators_f64: RwLock<
    HashMap<
      String,
      Box<
        dyn Fn(&ModelConfig) -> RegistryResult<Box<dyn BaseModel<f64>>>
          + Send
          + Sync,
      >,
    >,
...
impl ModelFactory {
  ...
  pub fn register_creator<T: Float>(
    ...
    _name: &str,
    _creator: Box<
      dyn Fn(&ModelConfig) -> RegistryResult<Box<dyn BaseModel<T>>>
        + Send
        + Sync,
    >,
  ) -> RegistryResult<()> {
    ...
    Err(RegistryError::UnsupportedOperation(
      "Generic creator registration not yet implemented without unsafe code"
        .to_string(),
    ))
  }

  /// Create a model from name
  pub fn create<T: Float>(
    &self,
    name: &str,
  ...
  pub fn create_with_options<T: Float>(
    ...
    // Update statistics
    {
      ...
      *stats.by_category.entry(model_info.category).or_insert(0) += 1;
      ...
      let avg_entry =
        stats.average_times.entry(name.to_string()).or_insert(0.0);
  ...
  pub fn create_from_config<T: Float>(
    &self,
    config: &ModelConfig,
  ) -> RegistryResult<Box<dyn BaseModel<T>>> {
  ...
  fn create_from_config_internal<T: Float>(
    ...
    Err(RegistryError::UnsupportedOperation(
      "Generic model creation not yet implemented without unsafe code"
        .to_string(),
    ))
  }

  /// Create multiple models in parallel
  pub fn create_models<T: Float>(
    &self,
    names: Vec<&str>,
  ...
  pub fn create_from_template<T: Float>(
    &self,
    template_name: &str,
  ) -> RegistryResult<Box<dyn BaseModel<T>>> {
  ...
  fn get_from_cache<T: Float>(
    &self,
    key: &str,
  ) -> Option<Box<dyn BaseModel<T>>> {
    ...
    if let Some(cached) = cache.get(key) {
      ...
      if let Some(_model) = cached.downcast_ref::<Box<dyn BaseModel<T>>>() {
        ...
        log::warn!(
          "Cache retrieval not fully implemented - model cloning needed"
        );
      }
    }
    None
  ...
  fn cache_model<T: Float>(&self, _key: &str, _model: &Box<dyn BaseModel<T>>) {
    ...
    log::debug!(
      "Model caching not fully implemented - needs Arc<Mutex<Model>>"
    );
  }
}
...
impl ModelFactory {
  ...
  pub fn create_from_config_global<T: Float>(
    config: &ModelConfig,
  ) -> RegistryResult<Box<dyn BaseModel<T>>> {
  ...
  pub fn create_models_global<T: Float>(
    names: Vec<&str>,
  ) -> RegistryResult<Vec<Box<dyn BaseModel<T>>>> {
