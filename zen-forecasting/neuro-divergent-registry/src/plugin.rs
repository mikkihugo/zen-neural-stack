...
use crate::{BaseModel, ModelConfig, ModelInfo, RegistryError, RegistryResult};
...
pub trait PluginInterface {
  ...
  fn create_model_f32(
    ...
    name: &str,
    config: &ModelConfig,
  ) -> RegistryResult<Box<dyn BaseModel<f32>>>;
  ...
  fn create_model_f64(
    ...
    name: &str,
    config: &ModelConfig,
  ) -> RegistryResult<Box<dyn BaseModel<f64>>>;
...
impl Plugin {
  ...
  pub fn load_from_path<P: AsRef<Path>>(
    ...
    {
      return Err(RegistryError::PluginError(format!(
    ...
    {
      return Err(RegistryError::PluginError(format!(
        "Plugin path {:?} is in blocked directories",
        path
      )));
    }
  ...
  pub fn load_from_descriptor<P: AsRef<Path>>(
    descriptor_path: P,
    config: PluginConfig,
  ) -> RegistryResult<Self> {
  ...
  fn load_descriptor<P: AsRef<Path>>(
    path: P,
  ) -> RegistryResult<PluginDescriptor> {
    let path = path.as_ref();
    ...
    if !descriptor_path.exists() {
      return Err(RegistryError::PluginError(format!(
        "Plugin descriptor not found at {descriptor_path:?}"
      )));
    }
    ...
    let descriptor: PluginDescriptor =
      toml::from_str(&content).map_err(|e| {
        RegistryError::PluginError(format!("Failed to parse descriptor: {e}"))
      })?;
  ...
  pub fn create_model_f32(
    ...
    if let Some(ref interface) = self.interface {
      ...
      Err(RegistryError::PluginError(
        "Plugin interface not available".to_string(),
      ))
    }
  }
  ...
  pub fn health_check(&self) -> RegistryResult<PluginHealth> {
    if let Some(ref interface) = self.interface {
      ...
      Ok(PluginHealth {
        ...
        errors: if self.is_active() {
          Vec::new()
        } else {
          vec!["Plugin not active".to_string()]
        },
        warnings: Vec::new(),
        metrics: HashMap::new(),
      })
...
impl PluginManager {
  ...
  pub fn load_plugin_from_descriptor<P: AsRef<Path>>(
    &self,
    descriptor_path: P,
  ...
  pub fn unload_plugin(&self, name: &str) -> RegistryResult<()> {
    ...
    if let Some(mut plugin) = plugins.remove(name) {
      ...
      Err(RegistryError::PluginError(format!(
  ...
  fn update_stats(&self) {
    ...
    stats.failed_plugins = plugins
      ...
      .count();
    stats.total_plugin_models =
      plugins.values().map(|p| p.descriptor.models.len()).sum();
  }
}
...
pub fn load_plugins_from_directory<P: AsRef<Path>>(
  dir: P,
) -> RegistryResult<Vec<Plugin>> {
  let dir = dir.as_ref();
  if !dir.exists() || !dir.is_dir() {
    return Err(RegistryError::PluginError(format!(
      "Plugin directory {dir:?} does not exist or is not a directory"
    )));
  }
