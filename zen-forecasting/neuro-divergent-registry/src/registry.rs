...
use crate::{
  ...
  BaseModel, Float, ModelCapabilities, ModelCategory, ModelConfig, ModelInfo,
  ModelPerformance, RegistryConfig, RegistryError, RegistryResult,
};
use ahash::AHashMap;
...
impl std::fmt::Debug for ModelDescriptor {
  fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    f.debug_struct("ModelDescriptor")
      ...
      .field(
        "creator_f32",
        &self.creator_f32.as_ref().map(|_| "<closure>"),
      )
      .field(
        "creator_f64",
        &self.creator_f64.as_ref().map(|_| "<closure>"),
      )
...
impl ModelRegistry {
  ...
  pub fn register<T: Float + 'static>(
    ...
    log::info!(
      "Registered model '{}' in category {:?}",
      name,
      descriptor.info.category
    );
    Ok(())
  }
  ...
  pub fn unregister_plugin(&self, name: &str) -> RegistryResult<()> {
    ...
    for model_info in &plugin.descriptor.models {
      if let Err(e) = self.unregister(&model_info.name) {
        log::warn!(
          "Failed to unregister model '{}' from plugin '{}': {}",
          model_info.name,
          name,
          e
        );
  ...
  fn extract_capabilities(
    ...
    if capabilities.online_learning {
      caps.push("online_learning".to_string());
    }
    if capabilities.batch_processing {
      caps.push("batch_processing".to_string());
    }
    if capabilities.streaming {
      caps.push("streaming".to_string());
    }
    if capabilities.multi_threading {
      caps.push("multi_threading".to_string());
    }
    if capabilities.gpu_acceleration {
      caps.push("gpu_acceleration".to_string());
    }
    if capabilities.quantization {
      caps.push("quantization".to_string());
    }
    if capabilities.pruning {
      caps.push("pruning".to_string());
    }
    if capabilities.fine_tuning {
      caps.push("fine_tuning".to_string());
    }
  ...
  fn matches_criteria(
    ...
    let model_capabilities =
      self.extract_capabilities(&descriptor.info.capabilities);
  ...
  fn update_stats(&self) {
    ...
    stats.average_creation_time_ms = if count > 0 {
