...
use crate::config::{
  Device, EarlyStoppingConfig, LossFunction, OptimizerType,
  PredictionIntervals, ScalerType, SchedulerConfig,
};
...
impl<T: Float> GenericModelBuilder<T> {
  ...
  pub fn parameter<K: Into<String>>(
    mut self,
    key: K,
    value: ConfigValue<T>,
  ) -> Self {
...
impl<T: Float + From<f32>> ModelBuilder<T, LSTMConfig<T>> for LSTMBuilder<T> {
  fn validate(&self) -> NeuroDivergentResult<()> {
    ...
    if self.dropout < NumCast::from(0.0f64).unwrap()
      || self.dropout >= NumCast::from(1.0f64).unwrap()
    {
      return Err(NeuroDivergentError::config(
...
impl<T: Float + From<f32>> ModelBuilder<T, NBEATSConfig<T>>
  for NBEATSBuilder<T>
{
  fn validate(&self) -> NeuroDivergentResult<()> {
    if self.horizon.is_none() {
...
mod tests {
  ...
  fn test_lstm_builder_validation() {
    let result = LSTMBuilder::<f32>::new().hidden_size(64).build();
