...
use crate::config::{
  CrossValidationConfig, Device, Frequency, PredictionIntervals, ScalerType,
};
...
impl<T: Float + Send + Sync> NeuralForecast<T> {
  ...
  pub fn fit_with_validation(
    ...
    if let Some(data_freq) = &data.frequency {
      if data_freq != &self.frequency {
        return Err(NeuroDivergentError::data(format!(
          "Data frequency {:?} does not match NeuralForecast frequency {:?}",
    ...
    let results: Vec<NeuroDivergentResult<()>> =
      if self.num_threads.unwrap_or(1) > 1 {
    ...
    if !errors.is_empty() {
      return Err(NeuroDivergentError::training(format!(
  ...
  pub fn predict_with_config(
    ...
    if !self.is_fitted {
      return Err(NeuroDivergentError::prediction(
        "Models have not been fitted yet",
      ));
    }
    ...
    let schema = self
      ...
      .as_ref()
      .ok_or_else(|| {
        NeuroDivergentError::prediction("No training schema available")
      })?
      .clone();
    ...
    Ok(ForecastDataFrame::new(
      ...
      self
        ...
        .as_ref()
        .map(|pi| pi.confidence_levels.clone()),
      schema,
    ))
  }
  ...
  pub fn load<P: AsRef<Path>>(_path: P) -> NeuroDivergentResult<Self> {
    ...
    Err(NeuroDivergentError::generic(
...
impl<T: Float + Send + Sync> NeuralForecastBuilder<T> {
  ...
  pub fn with_prediction_intervals(
    mut self,
...
mod tests {
  ...
  impl BaseModel<f32> for MockModel {
    ...
    fn predict(
      &self,
      _data: &TimeSeriesDataFrame<f32>,
    ) -> NeuroDivergentResult<ForecastDataFrame<f32>> {
  ...
  fn test_prediction_before_fitting() {
    let model = MockModel::new("test_model");
    let nf =
      NeuralForecast::new(vec![Box::new(model)], Frequency::Daily).unwrap();
