...
impl<T: Float> TimeSeriesDataFrame<T> {
  ...
  pub fn from_csv<P: AsRef<Path>>(
    ...
    let df = CsvReader::from_path(path.as_ref())
      .map_err(|e| {
        NeuroDivergentError::data(format!("CSV reader error: {}", e))
      })?
      .finish()
      .map_err(|e| {
  ...
  pub fn from_parquet<P: AsRef<Path>>(
    ...
    let df = LazyFrame::scan_parquet(path.as_ref(), ScanArgsParquet::default())
      .map_err(|e| {
        NeuroDivergentError::data(format!("Parquet read error: {}", e))
      })?
      .collect()
      .map_err(|e| {
        NeuroDivergentError::data(format!("Parquet collect error: {}", e))
      })?;
    ...
    let filtered = self
      ...
      .filter(
        col(&self.schema.ds_col)
          ...
          .and(col(&self.schema.ds_col).lt_eq(lit(end.timestamp_millis()))),
      )
      .collect()
      .map_err(|e| {
  ...
  pub fn time_range(
    ...
    let min_ts = ds_col.min::<i64>().ok_or_else(|| {
      NeuroDivergentError::data("Could not calculate min timestamp".to_string())
    })?;

    let max_ts = ds_col.max::<i64>().ok_or_else(|| {
      NeuroDivergentError::data("Could not calculate max timestamp".to_string())
    })?;
...
impl<T: Float> ForecastDataFrame<T> {
  ...
  pub fn get_model_forecasts(
    &self,
    model_name: &str,
  ) -> NeuroDivergentResult<DataFrame> {
    // Filter for specific model columns
    let model_cols: Vec<String> = self
      .data
      .get_column_names()
      .iter()
  ...
  pub fn to_point_forecasts(&self) -> DataFrame {
    ...
    let point_cols: Vec<String> = self
      .data
      .get_column_names()
      .iter()
      .filter(|col| {
  ...
  pub fn get_prediction_intervals(
    ...
    let interval_cols: Vec<String> = self
      .data
      .get_column_names()
      .iter()
      .filter(|col| {
        col.contains(model_name)
          && (col.contains("_lower")
            || col.contains("_upper")
            || col.contains("_q"))
      })
...
mod tests {
  ...
  fn test_forecast_dataframe_creation() {
    ...
    let forecast_df =
      ForecastDataFrame::<f32>::new(data, models, 2, None, schema);
