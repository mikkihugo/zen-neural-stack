...
use neuro_divergent::{
  ...
  config::{
    CrossValidationConfig, Device, EarlyStoppingConfig, LossFunction,
    OptimizerType, SchedulerConfig, SchedulerType,
  },
  data::{TimeSeriesDataFrame, TimeSeriesSchema},
  models::{DeepAR, LSTM, MLP, NBEATS, NHITS, RNN, TFT, Transformer},
};
...
fn test_python_auto_model_example() -> Result<(), Box<dyn std::error::Error>> {
  ...
  let mut nf = NeuralForecast::builder()
    .with_models(vec![Box::new(auto_lstm), Box::new(auto_nbeats)])
...
fn test_python_save_load() -> Result<(), Box<dyn std::error::Error>> {
  ...
  let lstm = LSTM::builder().horizon(12).input_size(24).build()?;
...
fn test_python_batch_prediction() -> Result<(), Box<dyn std::error::Error>> {
  ...
  let nbeats = NBEATS::builder().horizon(14).input_size(28).build()?;
...
fn test_output_format_compatibility() -> Result<(), Box<dyn std::error::Error>>
{
  ...
  let lstm = LSTM::builder().horizon(7).input_size(14).build()?;
...
-> Result<TimeSeriesDataFrame<f32>, Box<dyn std::error::Error>> {
  ...
  for i in 0..200 {
    ...
    values.push(
      100.0 + 10.0 * (i as f32 * 0.1).sin() + rand::random::<f32>() * 5.0,
    );
...
fn create_dataframe_with_exogenous()
-> Result<TimeSeriesDataFrame<f32>, Box<dyn std::error::Error>> {
...
fn create_multiple_series_dataframe(
  n_series: usize,
) -> Result<TimeSeriesDataFrame<f32>, Box<dyn std::error::Error>> {
