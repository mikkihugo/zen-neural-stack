...
fn test_expanding_window_cv() -> Result<(), Box<dyn std::error::Error>> {
  ...
  let models: Vec<Box<dyn BaseModel<f32>>> = vec![
    ...
    Box::new(NBEATS::builder().horizon(14).input_size(28).build()?),
  ];
...
fn test_model_selection_cv() -> Result<(), Box<dyn std::error::Error>> {
  // Generate data
  let data = generate_cv_test_data::<f32>(4, 300, true, Some(7))?;
  ...
  let candidate_models = vec![
    ...
    (
      "NBEATS_generic",
      Box::new(NBEATS::builder().horizon(7).input_size(14).build()?),
    ),
    (
...
fn test_hyperparameter_tuning_cv() -> Result<(), Box<dyn std::error::Error>> {
  // Generate data
  let data = generate_cv_test_data::<f32>(3, 250, true, Some(7))?;
    ...
    for &n_layers in &num_layers {
      for &dropout in &dropout_rates {
        for &lr in &learning_rates {
          ...
          // Run CV
          let cv_results =
            nf.cross_validation(data.clone(), cv_config.clone())?;
          let metrics = cv_results.calculate_metrics()?;
...
fn test_cv_loss_function_comparison() -> Result<(), Box<dyn std::error::Error>>
{
  ...
  let mut data = generate_cv_test_data::<f32>(2, 200, true, Some(7))?;

  // Add some outliers
  let mut df = data.to_polars()?;
  let mut y_values: Vec<f64> = df
    .column("y")?
    .f64()?
    .to_vec()
    .into_iter()
  ...
  data =
    TimeSeriesDataFrame::from_polars(df, data.schema.clone(), data.frequency)?;
...
fn test_ensemble_cross_validation() -> Result<(), Box<dyn std::error::Error>> {
  // Generate data
  let data = generate_cv_test_data::<f32>(5, 400, true, Some(7))?;
  ...
  let ensemble_mae: f32 = model_metrics.values().map(|m| m.mae()).sum::<f32>()
    / model_metrics.len() as f32;
...
fn test_cv_stability() -> Result<(), Box<dyn std::error::Error>> {
  // Generate data
  let data = generate_cv_test_data::<f32>(3, 200, true, Some(7))?;
...
fn test_cv_with_early_stopping() -> Result<(), Box<dyn std::error::Error>> {
  // Generate data
  let data = generate_cv_test_data::<f32>(3, 300, true, Some(7))?;
  ...
  let cv_results_es =
    nf_es.cross_validation(data.clone(), cv_config.clone())?;
...
fn test_cv_custom_metrics() -> Result<(), Box<dyn std::error::Error>> {
  // Generate data
  let data = generate_cv_test_data::<f32>(2, 200, true, Some(7))?;
