...
use neuro_divergent::{
  ...
  models::{DeepAR, LSTM, MLP, NBEATS, RNN, Transformer},
...
impl SeriesPattern {
    ...
    T: From<f64>,
  {
    ...
    // Add seasonality
    value += self.seasonal_amplitude
      * (2.0 * std::f64::consts::PI * t / self.seasonal_period).sin();
...
fn test_weighted_ensemble() -> Result<(), Box<dyn std::error::Error>> {
  ...
  let weighted_forecast =
    create_weighted_ensemble_forecast(individual_forecasts, model_weights)?;
...
fn test_ensemble_robustness() -> Result<(), Box<dyn std::error::Error>> {
  ...
  for i in 0..y_with_outliers.len() {
    if rng.gen_bool(0.05) {
      // 5% outlier rate
      y_with_outliers[i] =
        Some(y_with_outliers[i].unwrap_or(0.0) * rng.gen_range(2.0..5.0));
    }
  }
...
fn test_dynamic_model_selection() -> Result<(), Box<dyn std::error::Error>> {
  ...
  for series_id in &series_ids {
    ...
    println!(
      ...
      series_id, selected_models[series_id]
    );
  }
