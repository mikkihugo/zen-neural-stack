...
use neuro_divergent::{
  ...
  models::{DeepAR, LSTM, MLP, NBEATS, Transformer},
};
...
fn test_model_state_preservation() -> Result<(), Box<dyn std::error::Error>> {
  ...
  assert_eq!(
    loaded_nf.local_scaler_type,
    Some(ScalerType::StandardScaler)
  );
...
fn test_training_checkpoints() -> Result<(), Box<dyn std::error::Error>> {
  ...
  let restored_nf =
    NeuralForecast::<f32>::load_from_checkpoint(&latest_checkpoint.path())?;
