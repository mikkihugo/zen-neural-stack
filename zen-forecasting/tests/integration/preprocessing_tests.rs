...
use neuro_divergent::{
  ...
  data::{
    DataPreprocessor, DataTransform, DataValidator, LagsTransform,
    MinMaxScaler, RobustScaler, Scaler, StandardScaler, TimeSeriesDataFrame,
    TimeSeriesSchema, ValidationReport, WindowTransform,
  },
  models::{LSTM, NBEATS},
};
...
fn test_scaling_pipelines() -> Result<(), Box<dyn std::error::Error>> {
  ...
  let scalers: Vec<(String, Box<dyn Scaler<f32>>)> = vec![
    ...
    (
      ...
      Box::new(MinMaxScaler::new(0.0, 1.0)),
    ),
  ...
  for (scaler_name, scaler) in scalers {
    ...
    let original_forecasts =
      fitted_scaler.inverse_transform(&scaled_forecasts)?;
...
fn test_missing_value_handling() -> Result<(), Box<dyn std::error::Error>> {
  ...
  let nbeats = NBEATS::builder().horizon(7).input_size(14).build()?;
...
fn test_feature_engineering() -> Result<(), Box<dyn std::error::Error>> {
  // Generate base data
  let data = generate_preprocessing_test_data::<f32>(
    3, 365, // Full year for seasonal features
    0.0, 0.0,
  )?;
...
fn test_exogenous_preprocessing() -> Result<(), Box<dyn std::error::Error>> {
  // Generate base data
  let mut data = generate_preprocessing_test_data::<f32>(2, 200, 0.05, 0.0)?;
...
fn test_preprocessing_performance() -> Result<(), Box<dyn std::error::Error>> {
  ...
  let rows_per_second =
    data.shape().0 as f64 / preprocessing_time.as_secs_f64();
