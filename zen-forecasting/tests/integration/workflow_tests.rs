...
use neuro_divergent::{
  ...
  data::{MinMaxScaler, StandardScaler, TimeSeriesDataset},
    ...
    for (i, &date) in dates.iter().enumerate() {
      ...
      if with_seasonality {
        let seasonal_component =
          T::from((i as f64 * 2.0 * std::f64::consts::PI / 7.0).sin() * 5.0);
...
fn test_multi_model_ensemble_workflow() -> Result<(), Box<dyn std::error::Error>>
{
  ...
  let mut nf = NeuralForecast::builder()
    .with_models(vec![Box::new(lstm), Box::new(nbeats), Box::new(rnn)])
    .with_frequency(Frequency::Daily)
    .with_num_threads(2)
    .build()?;
...
fn test_workflow_with_preprocessing() -> Result<(), Box<dyn std::error::Error>>
{
  ...
  let mut data =
    generate_synthetic_data::<f32>(2, 100, Frequency::Hourly, true, false)?;
...
#[test]
fn test_workflow_with_exogenous_variables()
-> Result<(), Box<dyn std::error::Error>> {
  // Generate base data
  let mut data =
    generate_synthetic_data::<f32>(2, 100, Frequency::Daily, true, true)?;
  ...
  let temperatures: Vec<f32> =
    (0..n_rows).map(|_| rng.gen_range(15.0..30.0)).collect();
  ...
  let data_with_exog =
    TimeSeriesDataFrame::new(df, schema, Some(Frequency::Daily));
...
#[test]
fn test_workflow_with_prediction_intervals()
-> Result<(), Box<dyn std::error::Error>> {
  // Generate data
  let data =
    generate_synthetic_data::<f32>(1, 100, Frequency::Daily, true, true)?;

  // Create prediction intervals
  let intervals =
    PredictionIntervals::new(vec![0.80, 0.95], IntervalMethod::Quantile)?;
...
fn test_workflow_error_handling() {
  ...
  let lstm = LSTM::builder().horizon(7).input_size(14).build().unwrap();
...
fn test_workflow_with_custom_loss() -> Result<(), Box<dyn std::error::Error>> {
  // Generate data
  let data =
    generate_synthetic_data::<f32>(2, 100, Frequency::Daily, true, false)?;
...
fn test_workflow_with_evaluation() -> Result<(), Box<dyn std::error::Error>> {
  // Generate data
  let full_data =
    generate_synthetic_data::<f32>(2, 120, Frequency::Daily, true, true)?;
...
mod benchmarks {
  ...
  fn bench_prediction_performance() -> Result<(), Box<dyn std::error::Error>> {
    let data =
      generate_synthetic_data::<f32>(100, 200, Frequency::Daily, true, true)?;
