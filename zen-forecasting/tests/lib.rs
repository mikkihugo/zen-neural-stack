...
pub mod test_utils {
  ...
  pub fn generate_synthetic_series<T: Float>(
    ...
    match pattern {
      SeriesPattern::Linear { slope, intercept } => (0..length)
      ...
      SeriesPattern::Random { mean, std_dev } => {
        ...
        let normal =
          Normal::new(mean.to_f64().unwrap(), std_dev.to_f64().unwrap())
            .unwrap();
      ...
      SeriesPattern::Seasonal { period, amplitude } => (0..length)
        .map(|i| {
          let seasonal =
            T::from(i % period).unwrap() / T::from(period).unwrap();
          seasonal * amplitude
        })
        .collect(),
  ...
  pub fn create_test_dataframe<T: Float>(
    ...
    for series_idx in 0..n_series {
      ...
      let series_values =
        generate_synthetic_series(series_length, pattern.clone());
  ...
  pub fn generate_timestamps(
    ...
    (0..count).map(|i| start + duration * i as i32).collect()
  }
