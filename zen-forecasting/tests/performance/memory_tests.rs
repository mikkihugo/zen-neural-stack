...
impl fmt::Display for MemoryStats {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    ...
    write!(
      f,
      "  Memory Leaked: {:.2} MB",
      (self.allocated_bytes.saturating_sub(self.deallocated_bytes)) as f64
        / 1_048_576.0
    )
  }
}
...
fn test_model_creation_memory() {
  ...
  let (_, mlp_stats) = profile_memory!("MLP Creation", {
    let config = MLPConfig::new(100, 50).with_hidden_layers(vec![256, 128, 64]);
  ...
  assert!(
    mlp_stats.peak_usage < 10_000_000,
    "MLP uses too much memory"
  );
  assert!(
    lstm_stats.peak_usage < 20_000_000,
    "LSTM uses too much memory"
  );
  assert!(
    nbeats_stats.peak_usage < 30_000_000,
    "NBEATS uses too much memory"
  );
  assert!(
    tft_stats.peak_usage < 40_000_000,
    "TFT uses too much memory"
  );
}
...
fn test_training_memory_usage() {
  ...
  for batch_size in batch_sizes {
    let (_, stats) =
      profile_memory!(format!("Training with batch_size={}", batch_size), {
        ...
        trainer
          .train(&mut model, &training_data, &training_config)
          .unwrap()
      });
    ...
    assert!(
      stats.peak_usage < expected_max,
      "Training with batch_size={} uses too much memory",
      batch_size
    );
  }
}
...
fn test_data_processing_memory() {
  ...
  for size in data_sizes {
    ...
    let (_, stats) =
      profile_memory!(format!("Preprocessing {} samples", size), {
    ...
    assert!(
      stats.peak_usage < expected_max,
      "Preprocessing {} samples uses too much memory",
      size
    );
  }
}
...
fn test_memory_leak_detection() {
  ...
  for i in 0..iterations {
    ...
    for _ in 0..5 {
      let config = MLPConfig::new(50, 25).with_hidden_layers(vec![64, 32]);
  ...
  let first_half_avg: f64 = iteration_stats[..iterations / 2]
    .iter()
    .map(|&x| x as f64)
    .sum::<f64>()
    / (iterations / 2) as f64;
  let second_half_avg: f64 = iteration_stats[iterations / 2..]
    .iter()
    .map(|&x| x as f64)
    .sum::<f64>()
    / (iterations / 2) as f64;
...
fn test_batch_processing_memory_efficiency() {
  ...
  for batch_size in batch_sizes {
    let (_, stats) =
      profile_memory!(format!("Batch processing with size={}", batch_size), {
        ...
        for batch in loader {
          // Simulate processing
          let _sum: f64 = batch.iter().flat_map(|series| series.values()).sum();
          processed += batch.len();
        }
    ...
    assert!(
      stats.peak_usage < expected_max,
      "Batch processing with size={} uses too much memory",
      batch_size
    );
  }
}
...
fn test_model_parameter_memory() {
  ...
  for (size_name, layers) in layer_configs {
    let (model, stats) = profile_memory!(format!("MLP {} model", size_name), {
      let config = MLPConfig::new(100, 10).with_hidden_layers(layers.clone());
      MLP::new(config).unwrap()
    });
    ...
    println!(
      "  Expected memory: {:.2} MB",
      expected_memory as f64 / 1_048_576.0
    );
    println!(
      "  Actual peak: {:.2} MB",
    ...
    assert!(
      stats.peak_usage < (expected_memory as f64 * overhead_factor) as usize,
      "Model {} uses more memory than expected",
      size_name
    );
  }
}
...
fn generate_training_data(
  ...
  for series_idx in 0..num_series {
    ...
    for t in 0..length {
      ...
      for f in 0..features {
        let value =
          ((t + series_idx * 10) as f64 * 0.1 + f as f64).sin() * 10.0 + 50.0;
...
fn test_memory_pool_efficiency() {
  ...
  assert!(
    pooled_stats.allocation_count < unpooled_stats.allocation_count / 5,
