...
fn test_data_size_scaling() {
  ...
  for result in &results {
    assert!(
      result.scaling_efficiency >= efficiency_threshold,
...
fn test_model_complexity_scaling() {
  ...
  for (name, layers) in model_configs {
    ...
    // Create model
    let config = MLPConfig::new(24, 12).with_hidden_layers(layers.clone());
    let mut model = MLP::new(config).unwrap();
...
fn test_parallel_scaling() {
  ...
  let dataset =
    generate_multi_series_dataset(num_series, data_size / num_series);
  let mut results = Vec::new();
  ...
  for &num_threads in &thread_counts {
    ...
    let start = Instant::now();
    pool.install(|| {
      dataset.series.par_iter().zip(models.par_iter()).for_each(
        |(series, model)| {
          // Simulate training
          for _ in 0..10 {
  ...
  for result in &results {
    ...
    assert!(
      result.scaling_efficiency >= min_efficiency,
...
fn test_batch_size_scaling() {
  ...
  for i in 1..optimal_batch_idx {
    assert!(
      results[i].1.throughput >= results[i - 1].1.throughput * 0.95,
...
fn test_memory_scaling() {
  ...
  for &size in &data_sizes {
    ...
    let preprocess_memory =
      scaled.len() * std::mem::size_of::<f64>() as f64 / 1_048_576.0;
    ...
    let feature_memory = lag_features
      .iter()
    ...
    assert!(
      memory_per_sample < 1000.0,
      "Excessive memory usage per sample: {:.2} bytes",
      memory_per_sample
    );
  }
}
...
fn test_scalability_limits() {
  ...
  for (name, total_samples, num_series, seq_len) in test_configs {
    ...
    let config = if name.contains("Deep") {
      ...
      MLPConfig::new(seq_len, 12).with_hidden_layers(vec![128, 64])
    };
    ...
    assert!(
      model_time.as_secs() < 10,
      "{} model creation too slow",
      name
    );
    assert!(
...
fn generate_scaling_dataset(
  ...
  let values: Vec<f64> = (0..size)
    .map(|i| {
      ...
      let seasonal =
        (i as f64 * 2.0 * std::f64::consts::PI / 168.0).sin() * 10.0;
...
fn convert_dataset_to_training_data(
  dataset: &TimeSeriesDataset<f64>,
) -> TrainingData<f64> {
  let window_size = 24;
  let horizon = 12;
...
fn estimate_memory_usage<M: BaseModel<f64>>(
  model: &M,
  data_size: usize,
) -> f64 {
