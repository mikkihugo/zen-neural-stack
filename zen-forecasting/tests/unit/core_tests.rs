...
mod error_tests {
  ...
  fn test_error_context() {
    let base_error: NeuroDivergentResult<()> =
      Err(NeuroDivergentError::config("base error"));
...
mod forecast_tests {
  ...
  fn test_forecast_dataframe_creation() {
    ...
    let forecast_df =
      ForecastDataFrame::<f64>::new(data, models.clone(), 2, None, schema);
  ...
  fn test_model_forecast_extraction() {
    ...
    let forecast_df =
      ForecastDataFrame::<f64>::new(data, models, 2, None, schema);
...
mod integration_tests {
  ...
  fn test_end_to_end_data_workflow() {
    ...
    let ts_df =
      TimeSeriesDataFrame::<f64>::from_polars(data, schema.clone(), None);
    assert!(ts_df.is_ok());
    let ts_df = ts_df.unwrap();
