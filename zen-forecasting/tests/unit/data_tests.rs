...
impl<T: Float> MockStandardScaler<T> {
  ...
  pub fn transform(&self, data: &[T]) -> NeuroDivergentResult<Vec<T>> {
    ...
    let transformed = data.iter().map(|&x| (x - mean) / std).collect();
  ...
  pub fn inverse_transform(&self, data: &[T]) -> NeuroDivergentResult<Vec<T>> {
    ...
    let transformed = data.iter().map(|&x| x * std + mean).collect();
...
impl<T: Float> MockMinMaxScaler<T> {
  ...
  pub fn transform(&self, data: &[T]) -> NeuroDivergentResult<Vec<T>> {
    ...
    let scale =
      (self.feature_range.1 - self.feature_range.0) / (max_val - min_val);
...
impl MockFeatureEngineer {
  ...
  pub fn create_lags<T: Float + Copy>(
    data: &[T],
    lags: &[usize],
  ...
  pub fn rolling_mean<T: Float + Copy>(
    data: &[T],
    window: usize,
  ) -> Vec<Option<T>> {
  ...
  pub fn exponential_moving_average<T: Float + Copy>(
    data: &[T],
    alpha: T,
  ) -> Vec<T> {
    let mut result = vec![T::zero(); data.len()];
  ...
  pub fn seasonal_difference<T: Float + Copy>(
    data: &[T],
    period: usize,
  ) -> Vec<Option<T>> {
    let mut result = vec![None; data.len()];
  ...
  pub fn detect_outliers(&self, data: &[T]) -> NeuroDivergentResult<Vec<bool>> {
    match self.method {
      ...
      OutlierMethod::ModifiedZScore => {
        ...
        let mad = {
          let mut deviations: Vec<T> =
            data.iter().map(|&x| (x - median).abs()).collect();
...
mod io_tests {
  ...
  fn test_csv_export_import_roundtrip() {
    ...
    let schema = TimeSeriesSchema::default();
    let ts_df = TimeSeriesDataFrame::<f64>::from_polars(
      original_data,
      schema.clone(),
      None,
    )
    ...
    // Import from CSV
    let imported_ts_df =
      TimeSeriesDataFrame::<f64>::from_csv(temp_path, schema, None);
  ...
  fn test_schema_validation_with_missing_columns() {
    ...
    let schema = TimeSeriesSchema::default();
    let result =
      TimeSeriesDataFrame::<f64>::from_polars(invalid_data, schema, None);
  ...
  fn test_schema_with_exogenous_features() {
    ...
    let schema =
      TimeSeriesSchema::new("unique_id", "ds", "y").with_exogenous_features(
...
mod preprocessing_tests {
  ...
  fn test_standard_scaler() {
    ...
    let variance = scaled.iter().map(|&x| (x - mean) * (x - mean)).sum::<f64>()
      / scaled.len() as f64;
    let std = variance.sqrt();
...
mod imputation_tests {
  ...
  fn test_constant_imputation() {
    ...
    let mut imputer =
      MockDataImputer::new(MockImputationStrategy::Constant(-1.0));
...
mod integration_tests {
  ...
  fn test_complete_data_preprocessing_pipeline() {
    ...
    let clean_data: Vec<f64> = imputed
      .iter()
      .zip(outlier_flags.iter())
      .filter_map(
        |(&val, &is_outlier)| if !is_outlier { Some(val) } else { None },
      )
      .collect();
  ...
  fn test_time_series_data_workflow() {
    ...
    let ts_df =
      TimeSeriesDataFrame::<f64>::from_polars(data, schema, None).unwrap();
