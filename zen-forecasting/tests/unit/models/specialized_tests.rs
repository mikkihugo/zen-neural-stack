...
mod deepar_tests {
  ...
  fn test_likelihood_types() {
    ...
    for likelihood in likelihoods {
      let config = DeepARConfig::new(48, 24).with_likelihood(likelihood);
  ...
  fn test_categorical_embedding() {
    ...
    let embedder =
      CategoricalEmbedding::<f64>::new(&cardinality, &embedding_dim);
    ...
    let data = TimeSeriesData::new(
      (0..500)
        .map(|i| {
          ...
          let seasonal =
            (i as f64 * 2.0 * std::f64::consts::PI / 24.0).sin() * 5.0;
  ...
  fn test_deepar_with_covariates() {
    ...
    let dynamic_features =
      Array2::from_shape_fn((n_samples, n_dynamic_features), |(i, j)| {
        (i + j) as f64 * 0.01
      });

    let data = TimeSeriesData::new(target).with_exogenous(dynamic_features);
  ...
  fn test_deepar_multi_series() {
    ...
    for (idx, series) in series_data.iter().enumerate() {
      let data =
        TimeSeriesData::new(series.clone()).with_series_id(idx.to_string());
...
mod tcn_tests {
  ...
  fn test_tcn_workflow() {
    let data =
      TimeSeriesData::new((0..300).map(|i| (i as f64 * 0.05).sin()).collect());
...
mod bitcn_tests {
  ...
  fn test_bidirectional_convolution() {
    ...
    let biconv =
      BidirectionalConv1d::<f64>::new(in_channels, out_channels, kernel_size);
...
mod deepnpts_tests {
  ...
  fn test_prototype_network() {
    ...
    let prototype_net =
      PrototypeNetwork::<f64>::new(hidden_size, num_prototypes);
    ...
    assert_eq!(
      prototypes.shape(),
      &[batch_size, num_prototypes, hidden_size]
    );
    assert_eq!(
      similarities.shape(),
      &[batch_size, seq_length, num_prototypes]
    );
  ...
  fn test_memory_network() {
    ...
    let memory = MemoryNetwork::<f64>::new(memory_size, key_size, value_size);
  ...
  fn test_neural_process_layer() {
    ...
    let np_layer = NeuralProcessLayer::<f64>::new(hidden_size, latent_size);
...
mod common_specialized_tests {
  ...
  fn test_model_comparison() {
    let data =
      TimeSeriesData::new((0..200).map(|i| (i as f64 * 0.1).sin()).collect());

    let models: Vec<(&str, Box<dyn BaseModel<f64>>)> = vec![
      (
        "DeepAR",
        Box::new(
          DeepAR::new(DeepARConfig::new(40, 20).with_max_epochs(1)).unwrap(),
        ),
      ),
      (
        "TCN",
        Box::new(TCN::new(TCNConfig::new(40, 20).with_max_epochs(1)).unwrap()),
      ),
      (
        "BiTCN",
        Box::new(
          BiTCN::new(BiTCNConfig::new(40, 20).with_max_epochs(1)).unwrap(),
        ),
      ),
    ];
...
mod performance_tests {
  ...
  fn test_tcn_parallelization() {
    ...
    for (name, seq_length, channels) in configs {
      ...
      let config = TCNConfig::new(seq_length, 24).with_num_channels(channels);
  ...
  fn benchmark_specialized_models() {
    ...
    let data =
      TimeSeriesData::new((0..1000).map(|i| (i as f64 * 0.01).sin()).collect());
...
mod edge_case_tests {
  ...
  fn test_single_sample_deepar() {
    let config = DeepARConfig::new(10, 5).with_num_samples(1);
  ...
  fn test_single_channel_tcn() {
    let config = TCNConfig::new(20, 10).with_num_channels(vec![1]); // Single channel
  ...
  fn test_single_prototype() {
    let config = DeepNPTSConfig::new(20, 10).with_num_prototypes(1);
  ...
  fn test_no_dilation_tcn() {
    let config = TCNConfig::new(50, 25).with_num_channels(vec![32; 10]); // Many layers with dilation=1
  ...
  fn test_very_deep_tcn() {
    let config = TCNConfig::new(100, 50).with_num_channels(vec![8; 20]); // 20 layers
  ...
  fn test_extreme_kernel_size() {
    // Large kernel size
    let config = TCNConfig::new(100, 50).with_kernel_size(31);
...
mod integration_tests {
  ...
  fn test_ensemble_predictions() {
    ...
    let mut deepar =
      DeepAR::new(DeepARConfig::new(40, 20).with_max_epochs(2)).unwrap();

    let mut tcn = TCN::new(TCNConfig::new(40, 20).with_max_epochs(2)).unwrap();
    ...
    let ensemble_pred: Vec<f64> = deepar_pred
      .values
      .iter()
  ...
  fn test_hierarchical_forecasting() {
    ...
    // Aggregate to daily
    let daily_values: Vec<f64> = hourly_data
      .target
      .chunks(24)
    ...
    let hourly_model =
      TCN::new(TCNConfig::new(48, 24).with_max_epochs(1)).unwrap();

    let daily_model =
      TCN::new(TCNConfig::new(7, 7).with_max_epochs(1)).unwrap();
