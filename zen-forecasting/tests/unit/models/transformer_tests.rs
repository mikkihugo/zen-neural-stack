...
mod attention_tests {
  ...
  fn test_auto_correlation() {
    ...
    let input =
      Array3::from_shape_fn((batch_size, seq_length, d_model), |(b, t, d)| {
...
mod transformer_model_tests {
  ...
  fn test_transformer_validation() {
    ...
    // Zero layers
    let zero_layers = TransformerConfig::new(48, 24).with_n_encoder_layers(0);
  ...
  fn test_positional_encoding() {
    ...
    // Learnable encoding
    let learn_encoding =
      LearnablePositionalEncoding::<f64>::new(seq_length, d_model);
  ...
  fn test_transformer_workflow() {
    let data =
      TimeSeriesData::new((0..200).map(|i| (i as f64 * 0.05).sin()).collect());
    ...
    let data = TimeSeriesData::new(
      (0..400)
        .map(|i| {
          ...
          let seasonal =
            (i as f64 * 2.0 * std::f64::consts::PI / 24.0).sin() * 3.0;
...
mod tft_tests {
  ...
  fn test_temporal_self_attention() {
    ...
    let attention =
      InterpretableMultiHeadAttention::<f64>::new(d_model, n_heads);
  ...
  fn test_quantile_output() {
    ...
    let output_layer =
      QuantileOutput::<f64>::new(hidden_size, horizon, quantiles.clone());
  ...
  fn test_tft_workflow() {
    ...
    let data = TimeSeriesData::new(target).with_exogenous(known_features);
...
mod common_transformer_tests {
  ...
  fn test_feedforward_network() {
    ...
    let ffn =
      FeedForwardNetwork::<f64>::new(d_model, d_ff, ActivationType::ReLU, 0.1);
...
mod performance_tests {
  ...
  fn test_attention_complexity() {
    ...
    let configs =
      vec![("Short", 48, 64), ("Medium", 192, 128), ("Long", 720, 256)];
...
mod edge_case_tests {
  ...
  fn test_autoformer_no_decomposition() {
    ...
    let config = AutoformerConfig::new(10, 5).with_moving_avg_window(25);
