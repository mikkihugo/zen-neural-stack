...
use neuro_divergent::{
  AccuracyMetrics, NeuroDivergentError, NeuroDivergentResult,
};
...
impl MockModelFactory {
  ...
  pub fn create_model<T: Float>(
    &self,
    name: &str,
  ) -> NeuroDivergentResult<Box<dyn ModelBuilder<T>>> {
    let model_info = self.registry.get_model_info(name).ok_or_else(|| {
      NeuroDivergentError::config(format!(
    ...
    match model_info.model_type.as_str() {
      ...
      _ => Err(NeuroDivergentError::config(format!(
        "Unknown model type: {}",
...
trait ModelBuilder<T: Float>: Send + Sync {
  ...
  ) -> NeuroDivergentResult<Box<dyn TestableModel<T>>>;
...
impl<T: Float> ModelBuilder<T> for MockMLPBuilder {
  ...
  fn validate_config(
    ...
    if let Some(hidden_size_str) = config.get("hidden_size") {
      let hidden_size: usize = hidden_size_str.parse().map_err(|_| {
        NeuroDivergentError::config("Invalid hidden_size format")
      })?;
...
impl<T: Float> ModelBuilder<T> for MockLSTMBuilder {
  ...
  fn validate_config(
    ...
    if let Some(num_layers_str) = config.get("num_layers") {
      let num_layers: usize = num_layers_str.parse().map_err(|_| {
        NeuroDivergentError::config("Invalid num_layers format")
      })?;
...
mod registry_tests {
  ...
  fn test_model_registry_registration() {
    ...
    let model_info =
      MockModelInfo::new("TestMLP".to_string(), "MockMLP".to_string());
  ...
  fn test_model_registry_list_models() {
    ...
    let model2 =
      MockModelInfo::new("LSTM1".to_string(), "MockLSTM".to_string());
  ...
  fn test_model_registry_find_by_capability() {
    ...
    let mut model_info =
      MockModelInfo::new("SpecialModel".to_string(), "Special".to_string());
    model_info.capabilities =
      vec!["forecasting".to_string(), "clustering".to_string()];
    ...
    let missing_models =
      registry.find_models_by_capability("missing_capability");
...
mod factory_tests {
  ...
  fn test_model_factory_available_models() {
    ...
    let mlp_info =
      MockModelInfo::new("TestMLP".to_string(), "MockMLP".to_string());
    let lstm_info =
      MockModelInfo::new("TestLSTM".to_string(), "MockLSTM".to_string());
...
mod integration_tests {
  ...
  fn test_complete_registry_workflow() {
    ...
    let mlp_info =
      MockModelInfo::new("ProductionMLP".to_string(), "MockMLP".to_string());
    let lstm_info =
      MockModelInfo::new("ProductionLSTM".to_string(), "MockLSTM".to_string());
  ...
  fn test_registry_concurrent_access() {
    ...
    for i in 0..5 {
      ...
      let handle = thread::spawn(move || {
        let model_info =
          MockModelInfo::new(format!("Model{}", i), "MockMLP".to_string());
