...
fn benchmark_dnn_forward_pass(c: &mut Criterion) {
    ...
    for &size in &[config.small_network_size, config.medium_network_size, config.large_network_size] {
        ...
        // Create input data
        let input: Vec<f32> = (0..size.0).map(|i| (i as f32) / (size.0 as f32)).collect();
...
fn benchmark_activation_functions(c: &mut Criterion) {
    ...
    let test_values: Vec<f32> = (0..10000).map(|i| (i as f32 - 5000.0) / 1000.0).collect();
