...
unsafe impl GlobalAlloc for TrackingAllocator {
    unsafe fn alloc(&self, layout: Layout) -> *mut u8 {
        ...
        if !ptr.is_null() {
            let old_allocated = self.allocated.fetch_add(layout.size(), Ordering::Relaxed);
            let new_allocated = old_allocated + layout.size();
            let current_usage = new_allocated - self.deallocated.load(Ordering::Relaxed);
...
fn benchmark_js_memory_comparison(c: &mut Criterion) {
    ...
    group.bench_function("rust_memory_pattern", |b| {
        b.iter(|| {
            ...
            let input: Vec<f32> = (0..network_size.0).map(|i| i as f32 / network_size.0 as f32).collect();
    ...
    group.bench_function("javascript_style_memory_pattern", |b| {
        b.iter(|| {
            ...
            weights.push((0..network_size.1)
                .map(|_| (0..network_size.0).map(|_| 0.5).collect())
                .collect());
            weights.push((0..network_size.2)
