...
fn benchmark_vector_operations(c: &mut Criterion) {
    ...
    for size in &[256, 1024, 4096, 16384, 65536] {
        ...
        let b: Vec<f32> = (0..*size).map(|i| (i as f32 + 1.0) / *size as f32).collect();
...
fn benchmark_activation_functions_simd(c: &mut Criterion) {
    ...
    for size in &[1024, 4096, 16384] {
        let input: Vec<f32> = (0..*size).map(|i| (i as f32 - (*size as f32 / 2.0)) / (*size as f32 / 4.0)).collect();
...
fn benchmark_batch_processing_simd(c: &mut Criterion) {
    ...
    for &batch_size in batch_sizes {
        let batch_data: Vec<Vec<f32>> = (0..batch_size)
            .map(|_| (0..feature_size).map(|i| i as f32 / feature_size as f32).collect())
            .collect();
        ...
        group.bench_with_input(
            ...
            |b, _| {
                b.iter(|| {
                    let results: Vec<Vec<f32>> = batch_data.iter()
                        .map(|sample| {
        ...
        group.bench_with_input(
            ...
            |b, _| {
                ...
                b.iter(|| {
                    let results: Vec<Vec<f32>> = batch_data.par_iter()
                        .map(|sample| {
