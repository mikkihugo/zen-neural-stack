...
use zen_neural::gnn::{
    ...
    // Components
    aggregation::{AggregationStrategy, MeanAggregation, AttentionAggregation, MaxAggregation},
...
impl GNNApplication {
    ...
    async fn new(config: GNNExampleConfig) -> GNNResult<Self> {
        ...
        let storage = Arc::new(
            ZenUnifiedStorage::new(&config.storage_url).await
                .map_err(|e| GNNError::StorageError(format!("Storage initialization failed: {}", e)))?
        );
    ...
    async fn init_trainer(&mut self) -> GNNResult<()> {
        let training_config = TrainingConfig {
            ...
            mixed_precision: cfg!(feature = "gpu") && self.config.device.starts_with("gpu"),
            device: self.config.device.clone(),
            distributed: self.create_distributed_config().await?,
        };
        
        self.trainer = Some(
            GNNTrainer::new(training_config, self.graph_storage.clone()).await?
        );
    ...
    async fn create_distributed_config(&self) -> GNNResult<Option<DistributedConfig>> {
    ...
    async fn prepare_dataset(&self, dataset_name: &str) -> GNNResult<Vec<GraphData>> {
        if self.config.verbose {
    ...
    async fn create_synthetic_dataset(&self, num_graphs: usize) -> GNNResult<Vec<GraphData>> {
        if self.config.verbose {
    ...
    fn create_synthetic_graph(&self, seed: usize, num_nodes: usize, num_edges: usize) -> GNNResult<GraphData> {
        ...
        for node in 0..num_nodes {
            ...
            let max_connections = std::cmp::min(5, num_edges.saturating_sub(edge_count));
        ...
        ).map_err(|e| GNNError::ComputationError(format!("Node labels creation failed: {}", e)))?;
    ...
    async fn load_cora_dataset(&self) -> GNNResult<Vec<GraphData>> {
        ...
        let node_features = Array2::from_shape_vec((num_papers, num_features), node_data)
            .map_err(|e| GNNError::ComputationError(format!("Cora node features creation failed: {}", e)))?;
        ...
        ).map_err(|e| GNNError::ComputationError(format!("Cora labels creation failed: {}", e)))?;
        ...
        if self.config.verbose {
            println!("✅ Cora-like dataset loaded: {} papers, {} classes", num_papers, num_classes);
        }
    ...
    async fn load_citeseer_dataset(&self) -> GNNResult<Vec<GraphData>> {
        ...
        println!("ℹ️  CiteSeer dataset simulation - {} papers, {} features, {} classes",
...
impl GNNApplication {
    ...
    async fn run_training(&mut self, dataset: &str, model: &str) -> GNNResult<()> {
        println!("\n🎯 Starting Training");
        println!("===================");
        ...
        if self.config.verbose {
            ...
            if let Some(first_graph) = dataset.first() {
                println!("   Example Graph - Nodes: {}, Edges: {}", 
                         first_graph.node_features.0.dim().0,
                         first_graph.adjacency_list.iter().map(|adj| adj.len()).sum::<usize>());
            }
        }
        ...
        println!("📊 Train: {} graphs, Validation: {} graphs", 
        ...
        for (i, graph) in train_dataset.iter().enumerate() {
            ...
            self.graph_storage.store_graph(&graph_id, graph, Some("Training data".to_string())).await?;
        }
        ...
        let train_loader = GraphDataLoader::new(train_dataset, data_loader_config.clone())?;
        ...
        for epoch in 0..self.config.epochs {
            ...
            for (batch_idx, batch) in train_loader.clone().into_iter().enumerate() {
                ...
                if self.config.verbose && batch_idx % 10 == 0 {
                    println!("   Batch {}: loss={:.4}, time={}ms", 
                             batch_idx, batch_loss, batch_duration.as_millis());
                }
            }
            ...
            let avg_val_loss = if val_batches > 0 { epoch_val_loss / val_batches as f32 } else { avg_train_loss };
            let epoch_duration = epoch_start.elapsed();
            
            // Log epoch results
            println!("Epoch {}/{}: train_loss={:.4}, val_loss={:.4}, time={}s", 
                     epoch + 1, self.config.epochs, avg_train_loss, avg_val_loss, epoch_duration.as_secs());
            ...
            self.graph_storage.store_training_history("current_training", &history_entry).await?;
            ...
            if avg_val_loss < best_val_loss - 1e-4 {
                ...
                self.graph_storage.save_checkpoint("best_model", &best_checkpoint).await?;
                
                if self.config.verbose {
                    println!("   ✅ New best model saved (val_loss: {:.4})", avg_val_loss);
                }
            } else {
                patience_counter += 1;
            }
            ...
            if (epoch + 1) % 5 == 0 {
                ...
                self.graph_storage.save_checkpoint(&checkpoint_name, &checkpoint).await?;
        ...
        println!("Final Learning Rate: {:.6}", self.config.learning_rate * 0.95f32.powi(self.config.epochs as i32));
        ...
        self.graph_storage.store_training_history("training_summary", &summary).await?;
    ...
    async fn simulate_training_step(&self, batch: &[GraphData], batch_idx: usize) -> GNNResult<f32> {
        ...
        let simulated_loss = (base_loss + batch_variation + learning_progress).max(0.1);
    ...
    async fn simulate_validation_step(&self, batch: &[GraphData]) -> GNNResult<f32> {
...
impl GNNApplication {
    /// Run performance benchmarks
    async fn run_benchmarks(&self, bench_type: &str, iterations: usize) -> GNNResult<()> {
    ...
    async fn benchmark_storage(&self, iterations: usize) -> GNNResult<()> {
        ...
        for i in 0..iterations {
            ...
            self.graph_storage.store_graph(&graph_id, &sample_graph, None).await?;
        }
    ...
    async fn benchmark_aggregation(&self, iterations: usize) -> GNNResult<()> {
        ...
        let messages = Array2::from_shape_vec(
            ...
            vec![1.0; 128000],
        ).map_err(|e| GNNError::ComputationError(format!("Benchmark messages creation failed: {}", e)))?;
    ...
    async fn benchmark_training(&self, iterations: usize) -> GNNResult<()> {
        ...
        let data_loader = GraphDataLoader::new(dataset, DataLoaderConfig {
            batch_size: 16,
            shuffle: false,
            drop_last: false,
            num_workers: 1,
        ...
        for i in 0..iterations {
            for (batch_idx, batch) in data_loader.clone().into_iter().enumerate() {
                ...
                if batch_idx >= 10 { break; } // Limit batches per iteration
            }
    ...
    async fn benchmark_memory(&self, iterations: usize) -> GNNResult<()> {
        ...
        for i in 0..iterations {
            ...
            if (i + 1) % 20 == 0 {
                println!("   Iteration {}: {} nodes, {} MB memory", 
                         i + 1, size, (current_memory - initial_memory) / (1024 * 1024));
            }
        }
...
impl GNNApplication {
    ...
    async fn run_demo(&mut self) -> GNNResult<()> {
        ...
        println!("✅ Created demo graph: {} nodes, {} edges", 
                 demo_graph.node_features.0.dim().0,
                 demo_graph.adjacency_list.iter().map(|adj| adj.len()).sum::<usize>());
        
        // Store the graph
        self.graph_storage.store_graph("demo_graph", &demo_graph, Some("Interactive demo graph".to_string())).await?;
        ...
        let strategies = [
            ("Mean Aggregation", Box::new(MeanAggregation::new()) as Box<dyn AggregationStrategy>),
        ...
        ).map_err(|e| GNNError::ComputationError(format!("Demo messages creation failed: {}", e)))?;
        
        for (name, strategy) in strategies.iter() {
            let result = strategy.aggregate(&messages, &demo_graph.adjacency_list, 10)?;
        ...
        for epoch in 0..self.config.epochs {
            ...
            println!("   Epoch {}: loss={:.4}, time={}ms", 
                     epoch + 1, avg_loss, duration.as_millis());
        }
        ...
        println!("ℹ️  Distributed Training: DISABLED (compile with --features distributed)");
        ...
        for i in 0..10 {
            ...
            self.graph_storage.store_graph(&graph_id, &demo_graph, None).await?;
        }
        ...
        println!("    Explore more with: cargo run --example gnn_complete_example --help");
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut app = GNNApplication::new(config).await
