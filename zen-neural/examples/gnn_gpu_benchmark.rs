...
use zen_neural::gnn::{
    GraphData, GNNConfig, GNNModel, TrainingMode, 
    AggregationMethod, ActivationFunction,
...
impl GPUBenchmark {
    ...
    #[cfg(feature = "gpu")]
    async fn benchmark_gpu(&mut self, size: usize) -> Result<PerformanceResult, Box<dyn std::error::Error>> {
        if let Some(ref mut processor) = self.gpu_processor {
            ...
            result.memory_usage_mb = Some(stats.memory_stats.total_allocated as f64 / 1024.0 / 1024.0);
    ...
    async fn benchmark_cpu(&self, size: usize) -> Result<PerformanceResult, Box<dyn std::error::Error>> {
        let graphs = self.generate_test_graphs(size);
        let mut times = Vec::new();
        ...
        // Warmup
        let _ = cpu_model.forward(&graphs[0], TrainingMode::Inference).await?;
    ...
    #[cfg(feature = "gpu")]
    async fn benchmark_batch_gpu(&mut self, batch_size: usize, graph_size: usize) -> Result<PerformanceResult, Box<dyn std::error::Error>> {
        if let Some(ref mut processor) = self.gpu_processor {
    ...
    async fn run_single_graph_benchmarks(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        println!("📊 Single Graph Processing Performance");
        println!("{:-<80}", "");
        
        let header = format!(
            "{:<12} {:<15} {:<15} {:<15} {:<12} {:<10}",
            "Graph Size", "GPU Time (ms)", "CPU Time (ms)", "Speedup", "GPU TPS", "Memory (MB)"
        );
        println!("{}", header);
        println!("{:-<80}", "");
    ...
    #[cfg(feature = "gpu")]
    async fn run_batch_benchmarks(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        println!("{:<15} {:<15} {:<15} {:<15}", "Batch Config", "Avg Time (ms)", "Per Graph (ms)", "Total TPS");
        println!("{:-<60}", "");
    ...
    fn print_config(&self) {
        ...
        println!("   Hidden dimensions: {}", self.config.gnn_config.hidden_dimensions);
        println!("   Aggregation method: {:?}", self.config.gnn_config.aggregation);
        println!("   Activation function: {:?}", self.config.gnn_config.activation);
    }
}
...
async fn run_configuration_comparison() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let configurations = vec![
        ("Shallow (2 layers)", GNNConfig { num_layers: 2, ..base_config.gnn_config.clone() }),
        ("Medium (4 layers)", GNNConfig { num_layers: 4, ..base_config.gnn_config.clone() }),
        ("Deep (8 layers)", GNNConfig { num_layers: 8, ..base_config.gnn_config.clone() }),
        ("Wide (256 hidden)", GNNConfig { hidden_dimensions: 256, ..base_config.gnn_config.clone() }),
        ("Max Aggregation", GNNConfig { aggregation: AggregationMethod::Max, ..base_config.gnn_config.clone() }),
        ("Sum Aggregation", GNNConfig { aggregation: AggregationMethod::Sum, ..base_config.gnn_config.clone() }),
    ];
    
    println!("⚙️  GNN Configuration Comparison (1000-node graphs)");
    println!("{:-<60}", "");
    println!("{:<20} {:<15} {:<15}", "Configuration", "Time (ms)", "Throughput");
    println!("{:-<60}", "");
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match benchmark_mode {
        ...
        _ => {
            ...
            println!();
            println!("Usage: cargo run --example gnn_gpu_benchmark --features gpu [mode]");
        }
    }
...
mod tests {
    ...
    async fn test_benchmark_initialization() {
        ...
        assert!(benchmark.is_ok(), "Benchmark should initialize successfully");
    }
