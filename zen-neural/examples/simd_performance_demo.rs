...
use zen_neural::{NetworkBuilder, ActivationFunction};
use zen_neural::simd::{
    SimdBenchmarkSuite, BenchmarkConfig, quick_performance_test,
    SimdOpsFactory, SimdTrainingCoordinator, VectorSimdOps,
    HighPerfSimdOps, SimdMatrixOps
};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("====================================");
    println!("Targeting 50-100x improvement over JavaScript Float32Array operations\n");
    ...
    println!("Check the benchmark results above to see performance improvements.");
...
fn demonstrate_matrix_performance() {
    ...
    for &size in &sizes {
        ...
        let b: Vec<f32> = (0..size*size).map(|i| ((i + size) as f32) * 0.001).collect();
        ...
        println!("    Result sample: [{:.3}, {:.3}, {:.3}, ...]", c[0], c[1], c[2]);
        println!();
    }
}
...
fn demonstrate_vector_performance() {
    ...
    for &size in &sizes {
        ...
        println!("    Dot product: {:.2} μs (1000 iterations, result: {:.6})", 
                 dot_time.as_micros() as f64 / 1000.0, result / 1000.0);
        println!("    SAXPY: {:.2} μs (1000 iterations)", 
...
fn demonstrate_training_integration() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Average time per epoch: {:.2} ms", training_time.as_millis() as f64 / epochs as f64);

    // Test the trained network
    println!("\nTesting trained network:");
    for (i, (input, expected)) in training_data.inputs.iter().zip(training_data.outputs.iter()).enumerate() {
        let output = network.run(input);
        println!("  Pattern {}: Input {:?} → Output [{:.3}, {:.3}, {:.3}] (Expected {:?})", 
                 i + 1, input, output[0], output[1], output[2], expected);
    }
...
fn run_comprehensive_benchmarks() {
    ...
    if !matrix_results.is_empty() {
        ...
        let min_speedup = matrix_results.iter().map(|r| r.speedup).fold(f64::INFINITY, f64::min);
        
        println!("Matrix Operations Summary:");
        println!("  Average speedup: {:.1}x", avg_speedup);
        println!("  Range: {:.1}x - {:.1}x", min_speedup, max_speedup);
        println!("  Target (10-50x): {}", if avg_speedup >= 10.0 { "✅ ACHIEVED" } else { "⚠️ PARTIAL" });
    }
    ...
    if !vector_results.is_empty() {
        ...
        let min_speedup = vector_results.iter().map(|r| r.speedup).fold(f64::INFINITY, f64::min);
        
        println!("Vector Operations Summary:");
        println!("  Average speedup: {:.1}x", avg_speedup);
        println!("  Range: {:.1}x - {:.1}x", min_speedup, max_speedup);
        println!("  Target (50-100x): {}", if avg_speedup >= 50.0 { "✅ ACHIEVED" } else { "⚠️ PARTIAL" });
    }

    // Show detailed results for key operations
    println!("\nDetailed Results (selected operations):");
    for result in results.iter().take(10) {
        println!("  {}: {:.1}x speedup ({:.2} GFLOPS SIMD vs {:.2} GFLOPS scalar) {}",
                 result.operation,
                 result.speedup,
                 result.gflops_simd,
                 result.gflops_scalar,
                 if result.correctness_passed { "✅" } else { "❌" }
        );
    }

    // Performance analysis
    let total_results = results.len();
    let passed_correctness = results.iter().filter(|r| r.correctness_passed).count();
    ...
    println!("  Correctness pass rate: {:.1}% ({}/{})", 
             (passed_correctness as f64 / total_results as f64) * 100.0,
             passed_correctness, total_results);
    ...
    if avg_speedup >= 50.0 {
        ...
        println!("  📊 Progress toward 50-100x goal: {:.1}%", (avg_speedup / 50.0) * 100.0);
    }
}