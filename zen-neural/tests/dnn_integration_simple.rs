...
fn generate_test_data(samples: usize, input_dim: usize, output_dim: usize) -> Vec<DNNTrainingExample> {
    ...
    for _ in 0..samples {
        let input_data: Vec<f32> = (0..input_dim).map(|_| rng.r#gen::<f32>()).collect();
...
async fn test_dnn_training() {
    ...
    println!("   - Improvement: {:.1}%", ((initial_loss - final_loss) / initial_loss * 100.0).max(0.0));
...
async fn test_performance_large_network() {
    ...
    let mut model = builder.add_output_layer(output_size, ActivationType::Softmax).build().unwrap();
    ...
    println!("   - Network: {} -> {:?} -> {}", input_size, hidden_sizes, output_size);
    println!("   - Parameters: {}", model_info.parameter_count);
    println!("   - Total time ({} iterations): {:?}", num_iterations, total_time);
    println!("   - Average per forward pass: {:?}", avg_time);
    println!("   - Throughput: {:.0} inferences/sec", 1.0 / avg_time.as_secs_f64());
    
    // Verify reasonable performance
    assert!(avg_time.as_millis() < 10, "Forward pass should be fast even for large networks");
}
...
async fn test_memory_efficiency() {
    ...
    for (name, input_size, hidden_layers) in configs {
        ...
        let model = builder.add_output_layer(10, ActivationType::Softmax).build().unwrap();
        ...
        let efficiency_ratio = estimated_js_memory as f64 / info.memory_usage as f64;
        
        println!("   {} Network:", name);
        println!("     - Parameters: {}", info.parameter_count);
        println!("     - Rust memory: {} bytes ({:.2} MB)", info.memory_usage, info.memory_usage as f64 / 1_048_576.0);
        println!("     - Est. JS memory: {} bytes ({:.2} MB)", estimated_js_memory, estimated_js_memory as f64 / 1_048_576.0);
        println!("     - Efficiency gain: {:.1}x more efficient", efficiency_ratio);
        
        assert!(efficiency_ratio >= 2.0, "{} network should be at least 2x more memory efficient", name);
    }
}
...
async fn test_batch_processing() {
    ...
    for batch_size in batch_sizes {
        ...
        // Warm up
        for _ in 0..5 {
            let _ = model.forward(&batch_input, DNNTrainingMode::Inference).await.unwrap();
        }
        ...
        println!("   Batch size {}: {:.0} samples/sec (avg: {:?}/sample)", 
                batch_size, throughput, avg_per_sample);
        
        // Larger batches should be more efficient
        if batch_size > 1 {
            assert!(avg_per_sample.as_micros() < 1000, "Batch processing should be efficient");
        }
    }
}
...
async fn test_advanced_layers() {
    ...
    println!("   - Architecture: Modern activations + BatchNorm + LayerNorm + Dropout");
...
async fn test_performance_report() {
    ...
    println!("   ✓ Memory Usage: {:.2} MB", model_info.memory_usage as f64 / 1_048_576.0);
    ...
    println!("\n🎉 CONCLUSION:");
    println!("   The zen-neural DNN module successfully delivers high-performance");
    println!("   deep learning capabilities with {:.0} inferences/sec throughput", throughput);
    println!("   and efficient {:.2} MB memory usage. Ready for production use!", model_info.memory_usage as f64 / 1_048_576.0);
    
    println!("\n{}\n", "=".repeat(80));
    
    // Performance assertions
    assert!(throughput >= 1000.0, "Should achieve at least 1000 inferences/sec");
    assert!(avg_time.as_millis() < 10, "Average inference should be under 10ms");
}