...
impl JavaScriptStyleDNN {
    fn new(layer_sizes: &[usize], activation: JSActivationType, learning_rate: f32) -> Self {
        ...
        for window in layer_sizes.windows(2) {
            ...
            let biases: Vec<f32> = (0..output_size).map(|_| rng.r#gen::<f32>() * 0.1).collect();
    ...
    fn train_step(&mut self, input: &[f32], target: &[f32]) -> f32 {
        ...
        for i in 0..target.len() {
            ...
            let activation_derivative = match self.layers[output_layer_idx].activation {
                ...
                JSActivationType::ReLU => if z_values[output_layer_idx][i] > 0.0 { 1.0 } else { 0.0 },
        ...
        for l in (0..self.layers.len() - 1).rev() {
            ...
            for i in 0..self.layers[l].weights.len() {
                ...
                let activation_derivative = match self.layers[l].activation {
                    ...
                    JSActivationType::ReLU => if z_values[l][i] > 0.0 { 1.0 } else { 0.0 },
    ...
    fn train_batch(&mut self, inputs: &[Vec<f32>], targets: &[Vec<f32>], epochs: usize) -> Vec<f32> {
...
async fn test_dnn_forward_pass() {
    ...
    // Test forward pass
    let output = model.forward(&input, DNNTrainingMode::Inference).await.unwrap();
...
async fn test_dnn_training() {
    ...
    let config = DNNTrainingConfig {
        ...
        batch_size: 2,
        learning_rate: 0.1,
        optimizer: OptimizerConfig::Adam { beta1: 0.9, beta2: 0.999, epsilon: 1e-8 },
    ...
    assert!(final_loss < initial_loss, "Model should show learning progress");
}
...
async fn test_performance_comparison_forward_pass() {
    ...
    // Generate test input
    let input_vec: Vec<f32> = (0..input_size).map(|i| (i as f32) / (input_size as f32)).collect();
    ...
    for _ in 0..num_iterations {
        let _output = rust_model.forward(&rust_input, DNNTrainingMode::Inference).await.unwrap();
    }
    ...
    assert!(speedup_ratio >= 5.0, "Expected at least 5x speedup, got {:.2}x", speedup_ratio);
    
    // Verify outputs are numerically similar (within reasonable bounds)
    let rust_output = rust_model.forward(&rust_input, DNNTrainingMode::Inference).await.unwrap();
...
async fn test_performance_comparison_training() {
    ...
    for _ in 0..num_samples {
        let input_vec: Vec<f32> = (0..input_size).map(|_| rng.r#gen::<f32>()).collect();
        ...
        let target = DNNTensor::from_vec(target_vec.clone(), &target_shape).unwrap();
    ...
    let rust_config = DNNTrainingConfig {
        ...
        batch_size: 10,
        learning_rate: 0.01,
        optimizer: OptimizerConfig::SGD { momentum: 0.9, nesterov: false },
        loss_function: LossFunction::CrossEntropy,
    ...
    let rust_start = Instant::now();
    let rust_results = rust_model.train(rust_training_data, rust_config).await.unwrap();
    let rust_time = rust_start.elapsed();
    ...
    // Calculate performance ratio
    let training_speedup = js_time.as_nanos() as f64 / rust_time.as_nanos() as f64;
    ...
    assert!(training_speedup >= 8.0, "Expected at least 8x training speedup, got {:.2}x", training_speedup);
...
async fn test_memory_efficiency_comparison() {
    ...
    for (name, config) in layer_configs {
        println!("Testing memory efficiency for {} network: {:?}", name, config);
        ...
        let rust_model = builder.add_output_layer(*config.last().unwrap(), ActivationType::Softmax).build().unwrap();
        
        // Get memory statistics
        let model_info = rust_model.get_model_info();
        
        // Create JavaScript-style equivalent for comparison
        let js_model = JavaScriptStyleDNN::new(&config, JSActivationType::ReLU, 0.01);
        ...
        let memory_efficiency = js_memory_estimate as f64 / model_info.memory_usage as f64;
        
        println!("  Rust memory usage: {} bytes ({:.2} MB)", 
                model_info.memory_usage, 
                model_info.memory_usage as f64 / 1_048_576.0);
        println!("  Estimated JS memory: {} bytes ({:.2} MB)", 
                js_memory_estimate,
                js_memory_estimate as f64 / 1_048_576.0);
        println!("  Memory efficiency: {:.2}x more efficient", memory_efficiency);
        
        // Verify memory efficiency improvement
        assert!(memory_efficiency >= 1.5, "Expected at least 1.5x memory efficiency for {}", name);
    }
}
...
async fn test_scalability() {
    ...
    for (name, input_size, hidden_layers) in network_sizes {
        println!("Testing scalability for {} network (input: {})", name, input_size);
        ...
        for _ in 0..num_iterations {
            let _output = model.forward(&input, DNNTrainingMode::Inference).await.unwrap();
        }
        ...
        assert!(avg_time < Duration::from_millis(10), "Large networks should still be fast");
    }
}
...
async fn test_batch_processing_efficiency() {
    ...
    for batch_size in batch_sizes {
        ...
        let start = Instant::now();
        let output = model.forward(&batch_input, DNNTrainingMode::Inference).await.unwrap();
        ...
        println!("  Batch time: {:?}, Per sample: {:?}", batch_time, avg_per_sample);
        
        // Larger batches should be more efficient per sample
        if batch_size > 1 {
            assert!(avg_per_sample < Duration::from_millis(1), "Batch processing should be efficient");
        }
    }
}
...
async fn test_comprehensive_integration() {
    ...
    let config = DNNTrainingConfig {
        ...
        lr_scheduler: Some(LRSchedulerConfig::StepLR { step_size: 5, gamma: 0.5 }),
    ...
    assert!(results.history.len() > 0, "Training should complete at least one epoch");
    assert!(results.final_loss >= 0.0, "Final loss should be non-negative");
    assert_eq!(results.model_type, "dnn");
    ...
    let prediction = model.forward(&test_tensor, DNNTrainingMode::Inference).await.unwrap();
...
async fn test_generate_performance_report() {
    ...
    let js_model = JavaScriptStyleDNN::new(&[100, 50, 10], JSActivationType::ReLU, 0.01);
    ...
    let forward_speedup = js_forward_time.as_nanos() as f64 / rust_forward_time.as_nanos() as f64;
    ...
    let memory_efficiency = estimated_js_memory as f64 / model_info.memory_usage as f64;
    ...
    println!("  performance promises with {:.1}x forward pass speedup", forward_speedup);
    println!("  and {:.1}x memory efficiency improvement over JavaScript.", memory_efficiency);
    println!("  Ready for production use and training infrastructure integration.");
    
    println!("\n" + &"=".repeat(80));
    
    // Final assertions
    assert!(forward_speedup >= 5.0, "Forward pass should be at least 5x faster");
    assert!(memory_efficiency >= 1.5, "Memory usage should be at least 1.5x more efficient");
}