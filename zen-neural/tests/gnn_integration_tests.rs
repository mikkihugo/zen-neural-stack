...
use zen_neural::gnn::{
    ...
    training::{
        ...
        TrainingMetrics, ValidationConfig, EarlyStoppingConfig,
        CheckpointConfig, DistributedConfig
    },
    
    // Components
    aggregation::{AggregationStrategy, MeanAggregation, AttentionAggregation},
    updates::{NodeUpdate, GRUNodeUpdate, ResidualNodeUpdate},
    
    // Data loading
    data::{GraphDataLoader, DataLoaderConfig, BatchConfig}
};
...
impl GNNTestFixture {
    ...
    pub async fn new() -> GNNResult<Self> {
        ...
        let storage = Arc::new(
            ZenUnifiedStorage::new("memory://test_gnn_integration").await
                .map_err(|e| GNNError::StorageError(format!("Storage init failed: {}", e)))?
        );
...
fn create_sample_graph() -> GNNResult<GraphData> {
    ...
    ).map_err(|e| GNNError::ComputationError(format!("Node features creation failed: {}", e)))?;
    ...
    ).map_err(|e| GNNError::ComputationError(format!("Edge features creation failed: {}", e)))?;
...
async fn test_storage_integration() -> GNNResult<()> {
    ...
    // Store graph data
    fixture.graph_storage.store_graph(
        graph_id,
...
async fn test_aggregation_integration() -> GNNResult<()> {
    ...
    ).map_err(|e| GNNError::ComputationError(format!("Message creation failed: {}", e)))?;
    ...
    assert_eq!(aggregated.dim(), (5, 3), "Aggregated output should be (5, 3)");
    ...
    assert_eq!(attention_aggregated.dim(), (5, 3), "Attention aggregated output should be (5, 3)");
...
async fn test_node_update_integration() -> GNNResult<()> {
    ...
    let aggregated_messages = NodeFeatures(Array2::from_shape_vec(
        (5, 3),
        vec![0.1; 15], // Small values
    ).map_err(|e| GNNError::ComputationError(format!("Aggregated messages creation failed: {}", e)))?);
...
async fn test_training_pipeline_integration() -> GNNResult<()> {
    ...
    for i in 0..100 {
        ...
        graph.metadata.insert("graph_id".to_string(), json!(format!("graph_{}", i)));
        dataset.push(graph);
    }
    ...
    assert!(batches.len() <= 4, "Should create appropriate number of batches"); // ceil(100/32) = 4
...
async fn test_end_to_end_training_pipeline() -> GNNResult<()> {
    ...
    for epoch in 0..fixture.training_config.epochs {
        println!("Training epoch {}/{}", epoch + 1, fixture.training_config.epochs);
        ...
        let metrics = TrainingMetrics {
            ...
            learning_rate: fixture.training_config.learning_rate * 0.9f32.powi(epoch as i32),
            grad_norm: Some(0.1),
        ...
        if epoch == 1 {
            ...
            fixture.graph_storage.save_checkpoint("epoch_2_checkpoint", &checkpoint).await?;
        }
    }
    ...
    assert!(training_duration < Duration::from_secs(10), "Training should complete quickly in test");
    ...
    let final_checkpoint = ModelCheckpoint {
        ...
        metadata: HashMap::from([
            ...
            ("training_duration_ms".to_string(), json!(training_duration.as_millis())),
        ]),
    };
    
    fixture.graph_storage.save_checkpoint("final_model", &final_checkpoint).await?;
    
    // Verify final model can be loaded
    let loaded_final = fixture.graph_storage.load_checkpoint("final_model").await?;
    assert_eq!(loaded_final.epoch, fixture.training_config.epochs);
    assert!(loaded_final.accuracy.unwrap() > 0.8);
    
    println!("✅ End-to-end training pipeline test passed ({}ms)", training_duration.as_millis());
    Ok(())
}
...
async fn test_performance_benchmarks() -> GNNResult<()> {
    ...
    for i in 0..100 {
        ...
        fixture.graph_storage.store_graph(&graph_id, &fixture.sample_graph, None).await?;
    }
    ...
    assert!(storage_ops_per_sec > 50.0, "Storage should handle at least 50 ops/sec");
    ...
    let large_messages = Array2::from_shape_vec(
        ...
        vec![1.0; 128000],
    ).map_err(|e| GNNError::ComputationError(format!("Large messages creation failed: {}", e)))?;
    ...
    assert!(agg_ops_per_sec > 10.0, "Aggregation should handle at least 10 ops/sec");
    ...
    assert!(memory_per_graph < 10000, "Should use less than 10KB per graph");
    ...
    println!("📈 Target: Exceed original JavaScript performance (758 lines baseline)");
    println!("📈 Memory efficiency: {}x better than typical JS objects", 
...
async fn test_storage_persistence() -> GNNResult<()> {
    ...
    {
        let storage = Arc::new(ZenUnifiedStorage::new(storage_url).await
            .map_err(|e| GNNError::StorageError(format!("Storage init failed: {}", e)))?);
        
        let graph_storage = Arc::new(GraphStorage::new(storage, storage_config.clone()).await?);
        let sample_graph = create_sample_graph()?;
        
        // Store multiple graphs
        graph_storage.store_graph("persist_1", &sample_graph, Some("Test graph 1".to_string())).await?;
        graph_storage.store_graph("persist_2", &sample_graph, Some("Test graph 2".to_string())).await?;
        ...
        graph_storage.store_training_history("training_1", &history_entry).await?;
    }
    ...
    // Test 2: Recreate storage and verify data persistence
    {
        ...
        let graph_storage = Arc::new(GraphStorage::new(storage, storage_config).await?);
...
async fn test_gpu_acceleration() -> GNNResult<()> {
    use zen_neural::gpu::{GPUManager, DeviceType};
    ...
    println!("🚀 Testing GPU acceleration with {} devices", available_devices.len());
    
    // Test GPU memory allocation
    let device = &available_devices[0];
    let memory_info = gpu_manager.get_memory_info(device).await?;
    
    println!("📊 GPU Memory: {} MB total, {} MB free", 
    ...
    let messages = Array2::from_shape_vec(
        ...
        vec![1.0; 5000 * 256],
    ).map_err(|e| GNNError::ComputationError(format!("GPU messages creation failed: {}", e)))?;
    ...
    assert!(gpu_duration < Duration::from_millis(1000), "GPU should be fast");
...
async fn test_distributed_coordination() -> GNNResult<()> {
    ...
    let coordinator = DistributedCoordinator::new("127.0.0.1:9000".to_string()).await?;
    
    // Test node registration
    coordinator.register_node(NodeRole::Worker, "worker_1".to_string()).await?;
    coordinator.register_node(NodeRole::Worker, "worker_2".to_string()).await?;
    
    let active_nodes = coordinator.list_active_nodes().await?;
    assert!(active_nodes.len() >= 2, "Should have at least 2 worker nodes");
    ...
    let synchronized_grads = coordinator.all_reduce(&gradients, &dist_config).await?;
    ...
    assert!(remaining_nodes.len() >= 1, "Should handle node failures gracefully");
...
async fn test_distributed_coordination_disabled() -> GNNResult<()> {
    ...
    println!("ℹ️  To enable distributed tests, compile with: cargo test --features distributed");
    Ok(())
}
...
async fn test_memory_efficiency() -> GNNResult<()> {
    ...
    println!("📊 Memory for 10K node graph: {} bytes ({:.2} MB)", 
             graph_memory, graph_memory as f64 / (1024.0 * 1024.0));
    ...
    for batch_size in [16, 32, 64, 128] {
        ...
        println!("📊 Batch size {}: {} bytes per graph", 
                 batch_size, batch_memory / batch_size as u64);
    }
    ...
    for i in 1..batch_memories.len() {
        ...
        let variance = (curr_per_graph as f64 - prev_per_graph as f64).abs() / prev_per_graph as f64;
    ...
    assert!(memory_reclaimed > graph_memory / 2, "Should reclaim significant memory");
...
async fn test_resource_management() -> GNNResult<()> {
    ...
    for i in 0..50 {
        ...
        let handle = tokio::spawn(async move {
            ...
            let retrieved = storage_clone.retrieve::<serde_json::Value>(&key).await.unwrap();
    ...
    for handle in handles {
        handle.await.map_err(|e| GNNError::ComputationError(format!("Task failed: {}", e)))?;
    }
    ...
    // Test 2: Resource cleanup on drop
    {
        ...
        // Store some data
        temp_fixture.graph_storage.store_graph(
            "temp_graph", 
            &fixture.sample_graph, 
...
async fn test_error_handling() -> GNNResult<()> {
    ...
    let invalid_result = fixture.graph_storage.load_graph("non_existent_graph").await;
    assert!(invalid_result.is_err(), "Should fail for non-existent graph");
    ...
    let malformed_graph = GraphData {
        node_features: NodeFeatures(Array2::from_shape_vec((2, 3), vec![1.0; 6]).unwrap()),
        edge_features: None,
    ...
    let result = fixture.graph_storage.store_graph("malformed", &malformed_graph, None).await;
    ...
    match result {
        ...
        Err(GNNError::ValidationError(_)) => println!("✅ Malformed data correctly rejected"),
    ...
    let resource_test_result = async {
        ...
        for i in 0..100 {
            ...
            if get_memory_usage() > 500 * 1024 * 1024 { // 500MB limit
    ...
    assert!(resource_test_result.is_ok(), "Resource test should handle pressure gracefully");
...
fn create_large_sample_graph(num_nodes: usize, num_edges: usize) -> GNNResult<GraphData> {
    ...
    let node_features = Array2::from_shape_vec((num_nodes, 64), node_data)
        .map_err(|e| GNNError::ComputationError(format!("Large node features creation failed: {}", e)))?;
    ...
    let edge_features = Array2::from_shape_vec((num_edges, 32), edge_data)
        .map_err(|e| GNNError::ComputationError(format!("Large edge features creation failed: {}", e)))?;
    ...
    ).map_err(|e| GNNError::ComputationError(format!("Large node labels creation failed: {}", e)))?;
