...
async function runSwarmDemo() {
  ...
  for (const agentSpec of agentTypes) {
    for (let i = 0; i < agentSpec.count; i++) {
      ...
      console.log(
        `✅ Spawned agent: ${agent.name} (${agent.cognitive_pattern})`,
      );
    }
  }
  ...
  const diversityAnalysis =
    diversityEngine.analyze_swarm_diversity(agentComposition);
  console.log('\n📊 Swarm Diversity Analysis:');
  console.log(
    '- Overall diversity score:',
    diversityAnalysis.diversity_metrics.overall_diversity_score,
  );
  console.log(
    '- Pattern distribution:',
    diversityAnalysis.diversity_metrics.pattern_distribution,
  );
  ...
  const taskConfig = {
    description:
      'Research and implement a new neural network architecture for time series forecasting',
    priority: 'high',
  ...
  const trainingResult =
    neuralCoordinator.coordinate_neural_training(trainingConfig);
  ...
  for (const pattern of architecturePatterns) {
    const architecture =
      neuralArchitectures.get_architecture_for_pattern(pattern);
  ...
  console.log(
    '- Active connections:',
    monitoringData.real_time_metrics.active_connections,
  );
  console.log(
    '- Memory usage:',
    monitoringData.real_time_metrics.memory_usage_mb,
    'MB',
  );
  ...
  console.log(
    ...
    finalStatus.performance.avg_task_completion_time,
    'ms',
  );
