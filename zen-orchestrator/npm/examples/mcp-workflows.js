    ...
    return new Promise((resolve, reject) => {
      ...
      this.ws.on('open', async () => {
        try {
          ...
          console.log(
            `✅ Connected to RUV-Swarm MCP Server v${result.serverInfo.version}`,
          );
          resolve(result);
        } catch (error) {
          reject(error);
...
async function webAppDevelopmentWorkflow(client) {
  ...
  console.log(
    `  ✅ Architecture orchestration started: ${architectureResult.task_id}`,
  );
...
async function swarmCoordinationWorkflow(client) {
  ...
  for (const cluster of coordinatedTasks) {
    ...
    console.log(
      `    ✅ Created ${cluster.subtasks.length + 1} coordinated tasks`,
    );
  }
  ...
  client.on('ruv-swarm/event', (event) => {
    if (event.event.type === 'swarm_consensus') {
      ...
      console.log(
        `    🔄 Task handoff: ${event.event.data.from} → ${event.event.data.to}`,
      );
    }
  });
