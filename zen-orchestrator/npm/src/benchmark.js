...
class BenchmarkCLI {
  ...
  async run(args) {
    ...
    try {
      ...
      const avgSwarmTime =
        swarmTimes.reduce((a, b) => a + b, 0) / swarmTimes.length;
      ...
      if (testType === 'comprehensive' || testType === 'neural') {
        ...
        const avgNeuralTime =
          neuralTimes.reduce((a, b) => a + b, 0) / neuralTimes.length;
      ...
      const overallScore =
        (scores.length / Object.keys(results.benchmarks).length) * 100;
      ...
      if (results.benchmarks.neuralProcessing) {
        console.log(
          `   Neural Processing: ${results.benchmarks.neuralProcessing.status}`,
        );
      }
      ...
      for (const benchmark of benchmarks) {
        ...
        if (bench1 && bench2) {
          ...
          if (bench1.status && bench2.status) {
            const statusChange =
              bench1.status === bench2.status
                ? '='
                : bench1.status === 'PASS'
                  ? '📉'
                  : '📈';
            console.log(
              `  Status: ${bench1.status} → ${bench2.status} ${statusChange}`,
            );
          }
          console.log('');
        }
