...
class SwarmPersistencePooled {
  ...
  async createSwarm(swarm) {
    ...
    return this.trackOperation(() =>
      this.withRetry(async () => {
        ...
        const existing = await this.pool.read(
          'SELECT id FROM swarms WHERE id = ?',
          [swarm.id],
        );
  ...
  async getActiveSwarms() {
    ...
    return this.trackOperation(async () => {
      const swarms = await this.pool.read(
        'SELECT * FROM swarms WHERE status = ?',
        ['active'],
      );
      return swarms.map((s) => ({
        ...s,
  ...
  async updateAgentStatus(agentId, status) {
    ...
    return this.trackOperation(() =>
      this.withRetry(() =>
        this.pool.write('UPDATE agents SET status = ? WHERE id = ?', [
          status,
          agentId,
        ]),
      ),
  ...
  async getAgent(id) {
    ...
    return this.trackOperation(async () => {
      const agents = await this.pool.read('SELECT * FROM agents WHERE id = ?', [
        id,
      ]);
  ...
  async getTask(id) {
    ...
    return this.trackOperation(async () => {
      const tasks = await this.pool.read('SELECT * FROM tasks WHERE id = ?', [
        id,
      ]);
  ...
  async getAgentNeuralNetworks(agentId) {
    ...
    return this.trackOperation(async () => {
      const networks = await this.pool.read(
  ...
  async cleanup() {
    ...
    return this.trackOperation(async () => {
      ...
      await this.pool.write('DELETE FROM events WHERE timestamp < ?', [
        sevenDaysAgo,
      ]);
      ...
      await this.pool.write('DELETE FROM metrics WHERE timestamp < ?', [
        thirtyDaysAgo,
      ]);
