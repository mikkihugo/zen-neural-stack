...
class SQLiteConnectionPool extends EventEmitter {
  constructor(dbPath, options = {}) {
    ...
    this.options = {
      ...
      maxWorkers:
        options.maxWorkers || Math.max(2, Math.floor(os.cpus().length / 2)),
  ...
  async initializeWriteConnection() {
    try {
      ...
      throw new Error(
        `Failed to initialize write connection: ${error.message}`,
      );
    }
  }
  ...
  configureReadOnlyConnection(db) {
    ...
    try {
      ...
      console.debug(
        'Some pragma statements skipped for readonly connection:',
        error.message,
      );
    }
  }
  ...
  async acquireReaderConnection() {
    return new Promise((resolve, reject) => {
      if (this.availableReaders.length > 0) {
        ...
        // Timeout handling
        setTimeout(() => {
          const index = this.readQueue.findIndex(
  ...
  async processWorkerQueue() {
    if (this.workerQueue.length === 0 || this.availableWorkers.length === 0)
      return;

    const worker = this.availableWorkers.pop();
    const { sql, params, resolve, reject, timestamp } =
  ...
  async performHealthCheck() {
    ...
    try {
      ...
      this.isHealthy =
        healthyWorkers >= Math.floor(this.options.maxWorkers / 2);
      this.lastHealthCheck = Date.now();
