...
class LoadTestingSuite extends EventEmitter {
  ...
  async runGradualLoadTest() {
    ...
    try {
      ...
      for (let batch = 10; batch <= 60; batch += 10) {
        ...
        console.log(
          `     Spawned ${scenario.agents.length} agents in ${batchSpawnTime}ms`,
        );
  ...
  async runSustainedLoadTest() {
    ...
    try {
      ...
      console.log(
        `   Memory growth: ${Math.round(scenario.metrics.memoryGrowth / 1024 / 1024)}MB`,
      );
      console.log(
    ...
    try {
      ...
      for (let targetCount = 10; targetCount <= 150; targetCount += batchSize) {
        ...
        try {
          ...
          console.log(
            `     Successfully spawned ${currentAgents.length} agents`,
          );
          ...
          console.log(
            `     Spawn failures: ${scenario.metrics.agentSpawnFailures}`,
          );
          console.log(
            `     Execution failures: ${scenario.metrics.taskExecutionFailures}`,
          );
          ...
          if (memoryMB > 1000 || scenario.metrics.agentSpawnFailures > 10) {
            ...
            console.log(
              `   Failure point reached: ${scenario.failurePoint.reason}`,
            );
            break;
          }
          ...
          console.log(
            `   System error at ${currentAgents.length} agents: ${error.message}`,
          );
          break;
        }
      ...
      console.log(
        '   Failure point:',
        scenario.failurePoint || 'No failure reached',
      );
      console.log(
        `   Memory at end: ${scenario.metrics.memoryAtFailure.toFixed(1)}MB`,
      );
    } catch (error) {
      scenario.error = error.message;
      console.log(`   ❌ Failed: ${error.message}`);
  ...
  async generateLoadTestReport() {
    ...
    console.log(
      `Max Concurrent Agents: ${this.testResults.performance.maxConcurrentAgents}`,
    );
    console.log(
      `Average Response Time: ${this.testResults.performance.avgResponseTime}ms`,
    );
    console.log(`Error Rate: ${this.testResults.performance.errorRate}%`);
    console.log(
      `Peak Memory Usage: ${this.testResults.performance.memoryPeak.toFixed(1)}MB`,
    );
    console.log(
      `Overall Status: ${this.testResults.passed ? '✅ PASSED' : '❌ FAILED'}`,
    );
