...
async function main() {
  ...
  if (initial) {
    ...
    console.log('═══════════════════════════════════════════');
    console.log(
      `Lines:      ${initial.coverage.lines.toFixed(2)}% (${initial.details.lines})`,
    );
    console.log(
    ...
    if (initial.uncovered.length > 0) {
      ...
      initial.uncovered.slice(0, 5).forEach((file) => {
        console.log(
          `  - ${file.file}: ${file.lines.toFixed(1)}% lines covered`,
        );
      });
      console.log(
        `  ... and ${Math.max(0, initial.uncovered.length - 5)} more files\n`,
      );
    }
  }
