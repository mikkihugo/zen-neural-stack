...
const swarms = db
  .prepare(
    'SELECT id, name, topology, created_at FROM swarms ORDER BY created_at DESC LIMIT 5',
  )
  .all();
...
const agents = db
  .prepare(
    'SELECT id, name, type, created_at FROM agents ORDER BY created_at DESC LIMIT 5',
  )
  .all();
...
if (memoryCount.count > 0) {
  const memories = db
    .prepare(
      'SELECT key, agent_id, created_at FROM agent_memory ORDER BY created_at DESC LIMIT 3',
    )
    .all();
